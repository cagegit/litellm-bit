...
const EditBudgetModal: React.FC<EditBudgetModalProps> = ({ isModalVisible, setIsModalVisible, existingBudget }) => {
  ...
  const handleUpdate = async (formValues: Record<string, any>) => {
    try {
      ...
      NotificationsManager.fromBackend(t("errorUpdatingBudget") + `: ${error}`);
    }
  };

  return (
    <Modal title={t("editBudget")} open={isModalVisible} width={800} footer={null} onOk={handleOk} onCancel={handleCancel}>
      <Form
        form={form}
      ...
      >
        <>
          <Form.Item label={t("budgetId")} name="budget_id" help={t("budgetIdCannotChange")}>
