  ...
  return (
    <div className="space-y-3 bg-gray-50 p-4 rounded-lg">
      ...
      <div className="grid grid-cols-4 gap-4">
        <div>
          <Text className="text-xs text-gray-500 block">{t("totalPerRequest")}</Text>
          <Text className="text-base font-semibold text-blue-600">{formatCost(result.cost_per_request)}</Text>
        </div>
        <div>
          <Text className="text-xs text-gray-500 block">{t("inputCost")}</Text>
          <Text className="text-sm">{formatCost(result.input_cost_per_request)}</Text>
        </div>
        <div>
          <Text className="text-xs text-gray-500 block">{t("outputCost")}</Text>
          <Text className="text-sm">{formatCost(result.output_cost_per_request)}</Text>
        </div>
        <div>
          <Text className="text-xs text-gray-500 block">{t("marginFee")}</Text>
        ...
        <div className="grid grid-cols-4 gap-4 pt-2 border-t border-gray-200">
          <div>
            <Text className="text-xs text-gray-500 block">
              {`${periodLabel} ${t("total")} (${formatRequests(periodRequests)} ${t("req")})`}
            </Text>
          ...
          <div>
            <Text className="text-xs text-gray-500 block">{`${periodLabel} ${t("input")}`}</Text>
            <Text className="text-sm">{formatCost(periodInputCost)}</Text>
          </div>
          <div>
            <Text className="text-xs text-gray-500 block">{`${periodLabel} ${t("output")}`}</Text>
            <Text className="text-sm">{formatCost(periodOutputCost)}</Text>
          </div>
          <div>
            <Text className="text-xs text-gray-500 block">{`${periodLabel} ${t("marginFee")}`}</Text>
      ...
      {(result.input_cost_per_token || result.output_cost_per_token) && (
        <div className="text-xs text-gray-400 pt-2 border-t border-gray-200">
          ...
          {result.input_cost_per_token && (
            <span>
              {t("input")} ${formatNumberWithCommas(result.input_cost_per_token * 1_000_000, 2)}/1M
            </span>
          )}
          {result.input_cost_per_token && result.output_cost_per_token && " | "}
          {result.output_cost_per_token && (
            <span>
              {t("output")} ${formatNumberWithCommas(result.output_cost_per_token * 1_000_000, 2)}/1M
            </span>
          )}
        </div>
      )}
  ...
  if (!hasAnyResult && !isAnyLoading && !hasAnyError) {
    return (
      <div className="py-6 text-center border border-dashed border-gray-300 rounded-lg bg-gray-50">
        <Text className="text-gray-500">{t("selectModelsToSeeCostEstimates")}</Text>
      </div>
    );
  }
  ...
  if (!hasAnyResult && isAnyLoading && !hasAnyError) {
    return (
      <div className="py-6 text-center">
        ...
        <Text className="text-gray-500 block mt-2">{t("calculatingCosts")}</Text>
      </div>
    );
  }
    ...
    return (
      <div className="space-y-4">
        ...
        <div className="flex items-center justify-between">
          <Text className="text-base font-semibold text-gray-900">{t("costEstimates")}</Text>
      ...
      render: (
        ...
        <div className="flex flex-col gap-1">
          ...
          {record.hasZeroCost && !record.error && (
            <div className="text-xs text-amber-600 bg-amber-50 px-2 py-1 rounded">⚠️ {t("noPricingDataFound")}</div>
          )}
        </div>
      ),
    },
    ...
    <div className="space-y-4">
      ...
      <Card size="small" className="bg-gradient-to-r from-slate-50 to-blue-50 border-slate-200">
        <Row gutter={[16, 8]}>
          <Col xs={24} sm={12}>
            <Statistic
              title={<span className="text-xs">{t("totalPerRequest")}</span>}
          ...
          <Col xs={24} sm={12}>
            <Statistic
              title={<span className="text-xs">{`${t("total")} ${periodLabel}`}</span>}
        ...
        {hasMargin && (
          <Row gutter={[16, 8]} className="mt-3 pt-3 border-t border-slate-200">
            <Col xs={24} sm={12}>
              <div className="text-xs text-gray-500">{t("marginFeePerRequest")}</div>
              <div className="text-sm font-mono text-amber-600">
                {formatCost(multiResult.totals.margin_per_request)}
              </div>
            </Col>
            <Col xs={24} sm={12}>
              <div className="text-xs text-gray-500">{`${periodLabel} ${t("marginFee")}`}</div>
              <div className="text-sm font-mono text-amber-600">
