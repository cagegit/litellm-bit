...
const PassThroughSettings: React.FC<GeneralSettingsPageProps> = ({
  ...
  const confirmDelete = async () => {
    ...
    try {
      ...
      NotificationsManager.success(t("endpointDeleted"));
    } catch (error) {
      console.error("Error deleting the endpoint:", error);
      NotificationsManager.fromBackend(t("errorDeletingEndpoint") + error);
    }
    ...
    },
    {
      header: () => (
        <div className="flex items-center gap-1">
          <span>{t("methods")}</span>
          <Tooltip title={t("httpMethodsSupported")}>
      ...
      cell: (info: any) => {
        ...
        if (!methods || methods.length === 0) {
          return <Badge color="blue">{t("all")}</Badge>;
        }
        return (
    ...
    },
    {
      header: () => (
        <div className="flex items-center gap-1">
          ...
          <Tooltip title={t("virtualKeyRequired")}>
            <InformationCircleIcon className="w-4 h-4 text-gray-400 cursor-help" />
          </Tooltip>
        </div>
      ),
      accessorKey: "auth",
      cell: (info: any) => (
        <Badge color={info.getValue() ? "green" : "gray"}>{info.getValue() ? t("yes") : t("no")}</Badge>
      ),
    },
    {
      header: t("headers"),
  ...
  if (selectedEndpointId) {
    ...
    if (!selectedEndpoint) {
      return <div>{t("endpointNotFound")}</div>;
    }
  ...
  return (
    <div>
      <div>
        <Title>{t("passThroughEndpoints")}</Title>
        <Text className="text-tremor-content">{t("configurePassThroughEndpoints")}</Text>
      </div>
      ...
      <DataTable
        ...
        isLoading={false}
        noDataMessage={t("noPassThroughEndpoints")}
      />
              ...
              <div className="bg-white px-4 pt-5 pb-4 sm:p-6 sm:pb-4">
                <div className="sm:flex sm:items-start">
                  <div className="mt-3 text-center sm:mt-0 sm:ml-4 sm:text-left">
                    <h3 className="text-lg leading-6 font-medium text-gray-900">{t("deletePassThroughEndpoint")}</h3>
                    <div className="mt-2">
                      <p className="text-sm text-gray-500">{t("confirmDeletePassThroughEndpoint")}</p>
                    </div>
                  </div>
                </div>
              </div>
