  ...
  if (!accessGroup) {
    return (
      ...
      >
        ...
        <Empty description={t("accessGroupNotFound")} />
      </Content>
    );
  }
  ...
  const tabItems = [
    {
      ...
      children:
        modelIds?.length > 0 ? (
          ...
          <Empty description={t("noModelsAssigned")} />
        ),
    },
    {
      key: "mcp",
      ...
      children:
        mcpServerIds?.length > 0 ? (
          ...
          <Empty description={t("noMcpServersAssigned")} />
        ),
    },
    {
      key: "agents",
      ...
      children:
        agentIds?.length > 0 ? (
          ...
          <Empty description={t("noAgentsAssigned")} />
        ),
    },
  ];
  ...
  return (
    <Content style={{ padding: token.paddingLG, paddingInline: token.paddingLG * 2 }}>
      ...
      >
        <div style={{ display: "flex", alignItems: "center", gap: 16 }}>
          ...
          <div>
            ...
            <Text type="secondary">
              {t("id")}: <Text copyable>{accessGroup.access_group_id}</Text>
            </Text>
          </div>
        </div>
      ...
      <Row style={{ marginBottom: 24 }}>
        <Card>
          <Descriptions title={t("groupDetails")} column={1}>
            <Descriptions.Item label={t("description")}>{accessGroup.description || "—"}</Descriptions.Item>
            ...
            extra={
              keyIds?.length > MAX_PREVIEW ? (
                <Button type="link" onClick={() => setShowAllKeys(!showAllKeys)}>
                  {showAllKeys ? t("showLess") : t("viewAll", { count: keyIds?.length })}
                </Button>
              ) : null
            }
          >
            {keyIds?.length > 0 ? (
              ...
              <Empty description={t("noKeysAttached")} image={Empty.PRESENTED_IMAGE_SIMPLE} />
            )}
          </Card>
        </Col>
        <Col xs={24} lg={12}>
            ...
            extra={
              teamIds?.length > MAX_PREVIEW ? (
                <Button type="link" onClick={() => setShowAllTeams(!showAllTeams)}>
                  {showAllTeams ? t("showLess") : t("viewAll", { count: teamIds?.length })}
                </Button>
              ) : null
            }
          >
            {teamIds?.length > 0 ? (
              ...
              <Empty description={t("noTeamsAttached")} image={Empty.PRESENTED_IMAGE_SIMPLE} />
            )}
          </Card>
        </Col>
      </Row>
