...
const EmailSettings: React.FC<EmailSettingsProps> = ({ accessToken, premiumUser, alerts }) => {
  ...
  const handleSaveEmailSettings = async () => {
    ...
    try {
      ...
      NotificationManager.success(t("emailSettingsUpdated"));
    } catch (error) {
  ...
  return (
    <>
      ...
      <Card>
        <Title level={4}>{t("emailServerSettings")}</Title>
        <Text>
                      ...
                      <li key={key} className="mx-2 my-2">
                        ...
                        <p style={{ fontSize: "small", fontStyle: "italic" }}>
                          ...
                          {key === "TEST_EMAIL_ADDRESS" && (
                            <div style={{ color: "gray" }}>
                              {t("testEmailAddressDescription")}
                              <span style={{ color: "red" }}> {t("required")} </span>
                            </div>
                          )}
                          {key === "EMAIL_LOGO_URL" && (
                            <div style={{ color: "gray" }}>{t("emailLogoUrlDescription")}</div>
                          )}
                          {key === "EMAIL_SUPPORT_CONTACT" && (
                            <div style={{ color: "gray" }}>{t("emailSupportContactDescription")}</div>
                          )}
                        </p>
                      </li>
                    ))}
        ...
        <Button
          onClick={async () => {
            ...
            try {
              ...
              NotificationManager.success(t("emailTestTriggered"));
            } catch (error) {
              NotificationManager.fromBackend(error);
            }
          }}
