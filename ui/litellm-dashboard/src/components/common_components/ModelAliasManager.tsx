...
const ModelAliasManager: React.FC<ModelAliasManagerProps> = ({
  ...
  const handleAddAlias = () => {
    if (!newAlias.aliasName || !newAlias.targetModel) {
      NotificationsManager.fromBackend(t("provideAliasAndModel"));
      return;
    }

    // Check for duplicate alias names
    if (aliases.some((alias) => alias.aliasName === newAlias.aliasName)) {
      NotificationsManager.fromBackend(t("duplicateAlias"));
      return;
    }
    ...
    <div className="mt-4">
      <div className="mb-6">
        ...
        <div className="grid grid-cols-3 gap-4">
          ...
          <div>
            <label className="block text-xs text-gray-500 mb-1">{t("targetModel")}</label>
            <ModelSelector
              accessToken={accessToken}
              value={newAlias.targetModel}
              placeholder={t("selectTargetModel")}
              onChange={(value) =>
                setNewAlias({
                  ...newAlias,
                  targetModel: value,
          ...
          <Table className="[&_td]:py-0.5 [&_th]:py-1">
            <TableHead>
              <TableRow>
                ...
                <TableHeaderCell className="py-1 h-8">{t("targetModel")}</TableHeaderCell>
      ...
      {showExampleConfig && (
        <Card>
          ...
          <Text className="text-gray-600 mb-4">{t("configDescription")}</Text>
