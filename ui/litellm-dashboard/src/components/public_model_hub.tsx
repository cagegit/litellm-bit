...
const PublicModelHub: React.FC<PublicModelHubProps> = ({ accessToken, isEmbedded = false }) => {
    ...
    const initializeAndFetch = async () => {
      ...
      const fetchPublicData = async () => {
        try {
          ...
          setServiceStatus(t("serviceUnavailable"));
        } finally {
          setLoading(false);
        }
      };
  ...
  const formatLimits = (rpm?: number, tpm?: number) => {
    ...
    return limits.length > 0 ? limits.join(", ") : t("n/a");
  };
  ...
  const publicModelHubColumns = (): ColumnDef<ModelGroupInfo>[] => [
    ...
    },
    {
      ...
      cell: ({ row }) => {
        ...
        return <Text className="text-center">{cost ? formatCost(cost) : t("free")}</Text>;
      },
      size: 100,
      meta: {
        className: "text-center",
    ...
    },
    {
      ...
      cell: ({ row }) => {
        ...
        if (features.length === 0) {
          return <Text className="text-gray-400">{t("none")}</Text>;
        }
            ...
            <Tooltip
              title={
                <div className="space-y-1">
                  <div className="font-medium">{t("allFeatures")}:</div>
    ...
    },
    {
      ...
      cell: ({ row }) => {
        ...
        const responseTimeLabel = original.health_response_time
          ? `${t("responseTime")} ${Number(original.health_response_time).toFixed(2)}ms`
          : t("n/a");
        const lastCheckedLabel = original.health_checked_at
          ? `${t("lastChecked")} ${new Date(original.health_checked_at).toLocaleString()}`
          : t("n/a");
        ...
        return (
          ...
          >
            <Tag key={original.model_group} color={tagColor}>
              <span className="capitalize">{original.health_status ?? t("unknown")}</span>
            </Tag>
          </Tooltip>
        );
      },
  ...
  const publicAgentHubColumns = (): ColumnDef<AgentCard>[] => [
    ...
    },
    {
      ...
      cell: ({ row }) => {
        ...
        if (!provider) return <Text className="text-gray-400">{t("none")}</Text>;
        return (
        ...
        return (
          <div className="h-6 flex items-center space-x-1">
            ...
            <Tooltip
              title={
                <div className="space-y-1">
                  <div className="font-medium">{t("allSkills")}:</div>
  ...
  const publicMCPHubColumns = (): ColumnDef<MCPServerData>[] => [
    ...
    },
    {
      ...
      enableSorting: false,
      cell: ({ row }) => {
        const description = String(row.original.mcp_info?.description ?? t("none"));
        const truncated = description.length > 80 ? description.substring(0, 80) + "..." : description;
        return (
          <Tooltip title={description}>
            <Text className="text-sm text-gray-700">{truncated}</Text>
    ...
    <ThemeProvider accessToken={accessToken}>
      <div className={isEmbedded ? "w-full" : "min-h-screen bg-white"}>
        ...
        <div className={isEmbedded ? "w-full p-6" : "w-full px-8 py-12"}>
          ...
          {isEmbedded && (
            <div className="mb-6 p-4 bg-blue-50 border border-blue-200 rounded-lg">
              <p className="text-sm text-gray-700">{t("embeddedExplainer")}</p>
            </div>
          )}
          ...
          {!isEmbedded && (
            <Card className="mb-10 p-8 bg-white border border-gray-200 rounded-lg shadow-sm">
              ...
              <p className="text-gray-700 mb-6 text-base leading-relaxed">
                {customDocsDescription ? customDocsDescription : t("defaultDescription")}
              </p>
          ...
          {usefulLinks && Object.keys(usefulLinks).length > 0 && (
            <Card className="mb-10 p-8 bg-white border border-gray-200 rounded-lg shadow-sm">
              <Title className="text-2xl font-semibold mb-6 text-gray-900">{t("usefulLinks")}</Title>
          ...
          {!isEmbedded && (
            <Card className="mb-10 p-8 bg-white border border-gray-200 rounded-lg shadow-sm">
              <Title className="text-2xl font-semibold mb-6 text-gray-900">{t("healthAndEndpointStatus")}</Title>
          ...
          <Card className="p-8 bg-white border border-gray-200 rounded-lg shadow-sm">
            <Tabs activeKey={activeTab} onChange={setActiveTab} size="large" className="public-hub-tabs">
              {/* Models Tab */}
              <TabPane tab={t("modelHub")} key="models">
                <div className="flex justify-between items-center mb-8">
                  <Title className="text-2xl font-semibold text-gray-900">{t("availableModels")}</Title>
                </div>
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6 mb-8 p-6 bg-gray-50 rounded-lg border border-gray-200">
                  <div>
                    <div className="flex items-center space-x-2 mb-3">
                      <Text className="text-sm font-medium text-gray-700">{t("searchModels")}:</Text>
                      <Tooltip title={t("smartSearchTooltip")} placement="top">
                    ...
                    <div className="relative">
                      ...
                      <input
                        type="text"
                        placeholder={t("searchPlaceholderModels")}
                        value={searchTerm}
                ...
                <div className="mt-8 text-center">
                  <Text className="text-sm text-gray-600">
                    {t("showingModels", { shown: filteredData.length, total: modelHubData?.length || 0 })}
                  </Text>
                </div>
              </TabPane>
              ...
              {agentHubData && Array.isArray(agentHubData) && agentHubData.length > 0 && (
                <TabPane tab={t("agentHub")} key="agents">
                  <div className="flex justify-between items-center mb-8">
                    <Title className="text-2xl font-semibold text-gray-900">{t("availableAgents")}</Title>
                  </div>
                  ...
                  <div className="grid grid-cols-1 md:grid-cols-2 gap-6 mb-8 p-6 bg-gray-50 rounded-lg border border-gray-200">
                    <div>
                      <div className="flex items-center space-x-2 mb-3">
                        <Text className="text-sm font-medium text-gray-700">{t("searchAgents")}:</Text>
                        <Tooltip title={t("searchTooltipAgents")} placement="top">
                          <Info className="w-4 h-4 text-gray-400 cursor-help" />
                        </Tooltip>
                      </div>
                      <div className="relative">
                  ...
                  <div className="mt-8 text-center">
                    <Text className="text-sm text-gray-600">
                      {t("showingAgents", { shown: filteredAgentData.length, total: agentHubData?.length || 0 })}
                    </Text>
                  </div>
                </TabPane>
              )}
              ...
              {mcpHubData && Array.isArray(mcpHubData) && mcpHubData.length > 0 && (
                <TabPane tab={t("mcpHub")} key="mcp">
                  <div className="flex justify-between items-center mb-8">
                    <Title className="text-2xl font-semibold text-gray-900">{t("availableMcpServers")}</Title>
                  </div>
                  ...
                  <div className="grid grid-cols-1 md:grid-cols-2 gap-6 mb-8 p-6 bg-gray-50 rounded-lg border border-gray-200">
                    <div>
                      <div className="flex items-center space-x-2 mb-3">
                        <Text className="text-sm font-medium text-gray-700">{t("searchMcpServers")}:</Text>
                        <Tooltip title={t("searchTooltipMcp")} placement="top">
                          <Info className="w-4 h-4 text-gray-400 cursor-help" />
                        </Tooltip>
                      </div>
                      <div className="relative">
                        ...
                        <input
                          type="text"
                          placeholder={t("searchPlaceholderMcp")}
                          value={mcpSearchTerm}
                    ...
                    <div>
                      ...
                      <Select
                        ...
                        placeholder={t("selectTransportTypes")}
                        className="w-full"
                        size="large"
                        allowClear
                      >
                  ...
                  <div className="mt-8 text-center">
                    <Text className="text-sm text-gray-600">
                      {t("showingMcpServers", { shown: filteredMcpData.length, total: mcpHubData?.length || 0 })}
                    </Text>
                  </div>
                </TabPane>
              )}
              ...
              {/* Skill Hub Tab */}
              <TabPane tab={t("skillHub")} key="skills">
        ...
        <Modal
          title={
            <div className="flex items-center space-x-2">
              <span>{selectedModel?.model_group || t("modelDetails")}</span>
              {selectedModel && (
        ...
        >
          {selectedModel && (
            <div className="space-y-6">
                ...
                {selectedModel.model_group.includes("*") && (
                  <div className="bg-blue-50 border border-blue-200 rounded-lg p-4 mb-4">
                    <div className="flex items-start space-x-2">
                      ...
                      <div>
                        <Text className="font-medium text-blue-900 mb-2">{t("wildcardRouting")}</Text>
                        <Text className="text-sm text-blue-800 mb-2">
                          {t("wildcardRoutingDesc1")} <code className="bg-blue-100 px-1 py-0.5 rounded text-xs">*</code>{" "}
                          {t("wildcardRoutingDesc2")}
                        </Text>
                        <Text className="text-sm text-blue-800">
                          {t("forExampleWith")}{" "}
                          <code className="bg-blue-100 px-1 py-0.5 rounded text-xs">{selectedModel.model_group}</code>
                          {t("youCanUseAnyString")} (
                          <code className="bg-blue-100 px-1 py-0.5 rounded text-xs">
                            {selectedModel.model_group.replaceAll("*", "my-custom-value")}
                          </code>
                          ) {t("thatMatchesThisPattern")}
                        </Text>
                      </div>
                    </div>
                  </div>
              ...
              <div>
                <Text className="text-lg font-semibold mb-4">{t("tokenAndCostInformation")}</Text>
        ...
        <Modal
          title={
            <div className="flex items-center space-x-2">
              <span>{selectedAgent?.name || t("agentDetails")}</span>
              {selectedAgent && (
                <Tooltip title={t("copyAgentName")}>
                  <Copy
        ...
        >
          {selectedAgent && (
            <div className="space-y-6">
              ...
              {/* A2A Usage Example */}
              <div>
                <Text className="text-lg font-semibold mb-4">{t("usageExampleA2A")}</Text>

                {/* Step 1: Retrieve Agent Card */}
                <div className="mb-4">
                  <Text className="text-sm font-medium mb-2 text-gray-700">{t("step1RetrieveAgentCard")}</Text>
                  <div className="bg-gray-900 text-gray-100 p-4 rounded-lg overflow-x-auto">
    ...
    try:
        ...
        logger.warning(
            ...
            exc_info=True,
                ...
                <div>
                  <Text className="text-sm font-medium mb-2 text-gray-700">{t("step2CallAgent")}</Text>
                  <div className="bg-gray-900 text-gray-100 p-4 rounded-lg overflow-x-auto">
                    <pre className="text-xs">
                      {`client = A2AClient(
...
print(response.model_dump(mode='json', exclude_none=True))`;
        ...
        <Modal
          title={
            <div className="flex items-center space-x-2">
              <span>{selectedMcpServer?.server_name || t("mcpServerDetails")}</span>
              {selectedMcpServer && (
            ...
            <div className="space-y-6">
              ...
              <div>
                ...
                <div className="grid grid-cols-2 gap-4 mb-4">
                  ...
                  <div className="col-span-2">
                    <Text className="font-medium">{t("description")}:</Text>
                    <Text>{selectedMcpServer.mcp_info?.description || t("none")}</Text>
                  </div>
                  <div className="col-span-2">
                    <Text className="font-medium">{t("url")}:</Text>
                    <a
              ...
              {selectedMcpServer.mcp_info && Object.keys(selectedMcpServer.mcp_info).length > 0 && (
                <div>
                  <Text className="text-lg font-semibold mb-4">{t("additionalInformation")}</Text>
