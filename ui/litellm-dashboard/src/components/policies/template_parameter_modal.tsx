    ...
    <Modal
      title={
        <div>
          ...
          <p className="text-sm text-gray-500 font-normal">{t("configureCompetitorBlocking")}</p>
        </div>
      }
      open={visible}
      onCancel={onCancel}
      ...
      footer={[
        ...
        <Button key="confirm" onClick={handleConfirm} loading={isLoading} disabled={!canContinue || isLoading}>
          {isLoading ? t("creatingGuardrails") : t("continue")}
        </Button>,
      ]}
    >
      <div className="py-4 space-y-4">
        ...
        {hasEnrichment && (
          <>
            <div>
              <label className="block text-sm font-medium text-gray-700 mb-2">{t("competitorDiscovery")}</label>
              <Radio.Group
            ...
            <div>
              ...
              <TextInput
                placeholder={t("acmeAirlinesPlaceholder")}
            ...
            {competitorMode === "ai" && (
              <>
                ...
                <Button
                  ...
                  className="w-full"
                >
                  {isGenerating ? t("generatingNames") : t("generateCompetitorNames")}
                </Button>
              </>
            )}
            ...
            <div>
              ...
              <Select
                mode="tags"
                style={{ width: "100%" }}
                placeholder={t("typeNamePressEnter")}
                ...
                suffixIcon={null}
              />
              <p className="text-xs text-gray-500 mt-1">{t("typeNamePressEnterHint")}</p>
              {statusMessage && (
              ...
              {Object.keys(variationsMap).length > 0 && !statusMessage && (
                <p className="text-xs text-green-600 mt-1">
                  {t("alternateSpellingsGenerated", { count: Object.values(variationsMap).flat().length })}
                </p>
              )}
            </div>
            ...
            {competitorMode === "ai" && hasGenerated && competitorTags.length > 0 && (
              <div>
                <label className="block text-sm font-medium text-gray-700 mb-1">{t("refineList")}</label>
                ...
                <p className="text-xs text-gray-400 mt-1">{t("refineInstructions")}</p>
              </div>
            )}
          </>
        )}
