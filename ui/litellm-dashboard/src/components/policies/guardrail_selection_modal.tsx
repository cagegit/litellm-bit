...
const GuardrailSelectionModal: React.FC<GuardrailSelectionModalProps> = ({
  ...
  const guardrailsInfo: GuardrailInfo[] = (template?.guardrailDefinitions || []).map((def: any) => ({
    ...
    description: def.guardrail_info?.description || t("noDescriptionAvailable"),
  ...
  return (
    <Modal
          ...
          <div className="flex items-center gap-2">
            ...
            {progressInfo && (
              <span className="px-2 py-0.5 rounded-full text-xs font-medium bg-blue-50 text-blue-600 border border-blue-100">
                {t("templateProgress", { current: progressInfo.current, total: progressInfo.total })}
              </span>
            )}
          </div>
          <p className="text-sm text-gray-500 font-normal mt-1">{t("reviewGuardrails")}</p>
        </div>
      }
      open={visible}
      onCancel={onCancel}
      ...
      footer={[
        ...
        >
          {selectedCount > 0 ? t("createCountAndUseTemplate", { count: selectedCount }) : t("useTemplate")}
        </Button>,
      ]}
    >
      <div className="py-4">
          ...
          <div className="flex-1">
            <div className="text-sm">
              <span className="font-medium text-gray-900">
                {t("totalGuardrails", { count: guardrailsInfo.length })}
              </span>
              <span className="text-gray-600 mx-2">•</span>
              <span className="text-green-600 font-medium">{t("newGuardrails", { count: newGuardrailsCount })}</span>
              {existingCount > 0 && (
                <>
                  <span className="text-gray-600 mx-2">•</span>
                  <span className="text-gray-600">{t("alreadyExist", { count: existingCount })}</span>
                </>
              )}
            </div>
          </div>
              ...
              <div className="flex items-start gap-3">
                ...
                <div className="flex-1 min-w-0">
                  ...
                  <div className="flex gap-2 mt-2">
                    <Tag className="text-xs">{guardrail.definition?.litellm_params?.guardrail || t("unknown")}</Tag>
                    <Tag className="text-xs" color="blue">
                      {guardrail.definition?.litellm_params?.mode || t("unknown")}
                    </Tag>
                    {guardrail.definition?.litellm_params?.patterns && (
                      <Tag className="text-xs" color="purple">
                        {t("patternCount", { count: guardrail.definition.litellm_params.patterns.length })}
                      </Tag>
                    )}
                    {guardrail.definition?.litellm_params?.categories && (
                      <Tag className="text-xs" color="orange">
                        {t("categoryCount", { count: guardrail.definition.litellm_params.categories.length })}
                      </Tag>
                    )}
                  </div>
                </div>
        ...
        {guardrailsInfo.length === 0 && (
          <div className="text-center py-8 text-gray-500">
            <p>{t("noGuardrailsDefined")}</p>
            <p className="text-sm mt-2">{t("useExistingGuardrails")}</p>
          </div>
        )}
            ...
            <div className="p-3 bg-purple-50 rounded-lg border border-purple-100">
              <div className="flex items-center gap-2 mb-2">
                ...
                <span className="font-medium text-purple-900 text-sm">
                  {t("aiDiscoveredCompetitors", { count: template.discoveredCompetitors.length })}
                </span>
              </div>
              ...
              <p className="text-xs text-purple-600 mt-2">{t("competitorBlockerDescription")}</p>
            </div>
          </>
        )}
        ...
        <div className="text-sm text-gray-600">
          {selectedCount > 0 ? (
            <p>
              ...
              {t("guardrailsWillBeCreated", { count: selectedCount })}
            </p>
          ) : existingCount > 0 ? (
            <p className="text-green-600">{t("allGuardrailsExist")}</p>
          ) : (
            <p className="text-orange-600">{t("selectGuardrailPrompt")}</p>
          )}
        </div>
      </div>
    </Modal>
