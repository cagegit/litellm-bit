    ...
    >
      ...
      {inherits && (
        <div className="mb-4 text-xs">
          <span className="text-gray-500">{t("inheritsFrom")}: </span>
  ...
  useEffect(() => {
    const fetchTemplates = async () => {
      ...
      try {
        ...
        MessageManager.error(t("failedToFetchTemplates"));
      } finally {
        setIsLoading(false);
      }
    };
  ...
  if (isLoading) {
    return (
      <div className="flex justify-center items-center py-20">
        <Spin size="large" tip={t("loadingTemplates")} />
      </div>
    );
  }
  ...
  return (
    <div className="space-y-6">
      <div className="flex justify-between items-end">
        <div>
          <h2 className="text-lg font-medium text-gray-900">{t("policyTemplates")}</h2>
          <p className="text-sm text-gray-500 mt-1">{t("policyTemplatesDescription")}</p>
        </div>
          ...
          <div className="w-52 flex-shrink-0">
            <div className="sticky top-4">
              <div className="flex items-center justify-between mb-3">
                <span className="text-sm font-semibold text-gray-900">{t("categories")}</span>
                {selectedTags.size > 0 && (
        ...
        <div className="flex-1">
          {selectedTags.size > 0 && (
            <div className="mb-4 text-sm text-gray-500">
              {t("showingTemplates", { count: filteredTemplates.length, total: templates.length })}
            </div>
          )}
