  ...
  return (
    <div>
      <div className="bg-white border rounded-lg p-6 mb-6">
        <div className="mb-5">
          <h3 className="text-base font-semibold mb-1">{t("policySimulator")}</h3>
          <Text type="secondary">{t("simulateDescription")}</Text>
        </div>

        <Form form={form} layout="vertical">
          <div className="grid grid-cols-2 gap-4">
            <Form.Item name="team_alias" label={t("teamAlias")} className="mb-3">
              <Select
                showSearch
                allowClear
                placeholder={t("selectTeamAlias")}
                options={availableTeams.map((t) => ({ label: t, value: t }))}
                filterOption={(input, option) => (option?.label ?? "").toLowerCase().includes(input.toLowerCase())}
              />
            </Form.Item>
            <Form.Item name="key_alias" label={t("keyAlias")} className="mb-3">
              <Select
                showSearch
                allowClear
                placeholder={t("selectKeyAlias")}
      ...
      {!hasSearched && (
        <div className="bg-white border rounded-lg p-8 text-center">
          ...
          <p className="text-sm font-medium text-gray-600 mb-1">{t("noSimulationRun")}</p>
          <p className="text-xs text-gray-400">{t("noSimulationRunDescription")}</p>
        </div>
      )}
      ...
      {hasSearched && result && (
        <div className="bg-white border rounded-lg p-6">
          {result.matched_policies.length === 0 ? (
            <Empty description={t("noPoliciesMatched")} />
          ) : (
            <>
              <div className="mb-4">
                <p className="text-sm font-semibold mb-2">{t("effectiveGuardrails")}</p>
              ...
              <div>
                <p className="text-sm font-semibold mb-2">{t("matchedPolicies")}</p>
                <table className="w-full text-sm">
                  <thead>
                    <tr className="border-b">
                      <th className="text-left py-2 pr-4">{t("policy")}</th>
                      <th className="text-left py-2 pr-4">{t("matchedVia")}</th>
                      <th className="text-left py-2">{t("guardrailsAdded")}</th>
                    </tr>
                  </thead>
                  <tbody>
      ...
      {hasSearched && !result && !isLoading && (
        <Alert message={t("error")} description={t("failedToResolvePolicies")} type="error" showIcon />
      )}
    </div>
  );
};
