...
const PolicyInfoView: React.FC<PolicyInfoViewProps> = ({
  ...
  if (!policy) {
    return (
      <Card>
        <Text type="danger">{t("policyNotFound")}</Text>
        <br />
    ...
    <Card>
      <div className="space-y-6">
        ...
        <Descriptions bordered column={1}>
          <Descriptions.Item label={t("policyId")}>
            <code className="text-xs bg-gray-100 px-2 py-1 rounded">{policy.policy_id}</code>
          </Descriptions.Item>
          <Descriptions.Item label={t("description")}>
            {policy.description || <Text type="secondary">{t("noDescription")}</Text>}
          </Descriptions.Item>
          <Descriptions.Item label={t("inheritsFrom")}>
            {policy.inherit ? (
        ...
        {policy.pipeline && (
          <>
            <Divider orientation="left">
              <Text strong>{t("pipelineFlow")}</Text>
            </Divider>
            <Alert
              message={t("pipelineSummary", { mode: policy.pipeline.mode, count: policy.pipeline.steps.length })}
              type="info"
              showIcon
        ...
        <Divider orientation="left">
          <Text strong>{t("guardrailsConfiguration")}</Text>
        </Divider>
        ...
        <Descriptions bordered column={1}>
          <Descriptions.Item label={t("guardrailsToAdd")}>
            <div className="flex flex-wrap gap-1">
          ...
          <Descriptions.Item label={t("guardrailsToRemove")}>
            <div className="flex flex-wrap gap-1">
        ...
        <Descriptions bordered column={1}>
          <Descriptions.Item label={t("modelCondition")}>
            {policy.condition?.model ? (
              ...
              <Text type="secondary">{t("noModelCondition")}</Text>
            )}
          </Descriptions.Item>
        </Descriptions>
      </div>
