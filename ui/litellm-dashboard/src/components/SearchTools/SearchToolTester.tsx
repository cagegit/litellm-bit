...
export const SearchToolTester: React.FC<SearchToolTesterProps> = ({ searchToolName, accessToken, className = "" }) => {
  ...
  const handleSearch = async () => {
    if (!query.trim()) {
      MessageManager.warning(t("searchQueryRequired"));
      return;
    }
    ...
    try {
      ...
      NotificationsManager.fromBackend(t("searchFailed"));
    } finally {
      setIsLoading(false);
    }
  };
  ...
  const clearHistory = () => {
    setSearchHistory([]);
    setExpandedResults({});
    NotificationsManager.success(t("historyCleared"));
  };
  ...
  return (
    <Card className="mt-6">
      <div className="mb-6">
        <TremorTitle>{t("testSearchTool")}</TremorTitle>
      </div>
        ...
        <div className="flex-1">
          {!latestResults && !isLoading ? (
            <div className="h-full flex flex-col items-center justify-center p-8">
              ...
              <Text className="text-lg text-gray-600 font-medium">{t("testYourSearchTool")}</Text>
              <Text className="text-sm text-gray-500 mt-2">{t("enterQueryToSeeResults")}</Text>
            </div>
          ) : (
            <div>
              {isLoading && (
                ...
                <>
                  ...
                  latestResults.response.results.length > 0 ? (
                          ...
                          >
                            <div className="p-5">
                              ...
                              {result.snippet.length > 200 && (
                                ...
                                >
                                  {isResultExpanded ? t("showLess") : t("showMore")}
                                </Button>
                              )}
                            </div>
                          </div>
                    ...
                    <div className="text-center py-12 bg-gray-50 border border-gray-200 rounded-lg">
                      ...
                      <Text className="text-gray-600 font-medium">{t("noResultsFound")}</Text>
                      <Text className="text-sm text-gray-500 mt-1">{t("tryDifferentQuery")}</Text>
                    </div>
                  )}
                </>
              )}
              ...
              {searchHistory.length > 1 && (
                <div className="mt-8 pt-6 border-t border-gray-200">
                  <div className="flex items-center justify-between mb-4">
                    <Text className="text-sm font-semibold text-gray-700">{t("previousSearches")}</Text>
                    <Button
