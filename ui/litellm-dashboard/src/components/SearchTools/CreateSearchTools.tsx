...
const CreateSearchTool: React.FC<CreateSearchToolProps> = ({
  ...
  const handleCreate = async (formValues: Record<string, any>) => {
    ...
    try {
      ...
      if (accessToken != null) {
        ...
        NotificationsManager.success(t("searchToolCreated"));
        form.resetFields();
        setFormValues({});
      ...
      NotificationsManager.error(t("errorCreatingSearchTool") + error);
    } finally {
      setIsLoading(false);
    }
  };
  ...
  const handleTestConnection = async () => {
    try {
      ...
      NotificationsManager.error(t("fillRequiredFields"));
    }
  };
    ...
    <Modal
      title={
        <div className="flex items-center space-x-3 pb-4 border-b border-gray-100">
          ...
          <h2 className="text-xl font-semibold text-gray-900">{t("addNewSearchTool")}</h2>
        </div>
      }
      open={isModalVisible}
      width={800}
      ...
      <div className="mt-6">
        ...
        >
          <div className="grid grid-cols-1 gap-6">
            <Form.Item
              label={
                <span className="text-sm font-medium text-gray-700 flex items-center">
                  {t("searchToolName")}
                  <Tooltip title={t("searchToolNameTooltip")}>
              ...
              rules={[
                { required: true, message: t("pleaseEnterSearchToolName") },
                {
                  pattern: /^[a-zA-Z0-9_-]+$/,
                  message: t("nameValidationPattern"),
                },
              ]}
            >
              <TextInput
                placeholder={t("searchToolNamePlaceholder")}
            ...
            <Form.Item
              label={
                <span className="text-sm font-medium text-gray-700 flex items-center">
                  {t("searchProvider")}
                  <Tooltip title={t("searchProviderTooltip")}>
                    <InfoCircleOutlined className="ml-2 text-blue-400 hover:text-blue-600 cursor-help" />
                  </Tooltip>
                </span>
              }
              name="search_provider"
              rules={[{ required: true, message: t("pleaseSelectSearchProvider") }]}
            >
              <Select
                placeholder={t("selectSearchProviderPlaceholder")}
            ...
            <Form.Item
              label={
                <span className="text-sm font-medium text-gray-700 flex items-center">
                  {t("apiKey")}
                  <Tooltip title={t("apiKeyTooltip")}>
                    <InfoCircleOutlined className="ml-2 text-blue-400 hover:text-blue-600 cursor-help" />
                  </Tooltip>
                </span>
              }
              name="api_key"
              rules={[{ required: false, message: t("pleaseEnterApiKey") }]}
            >
              <TextInput
                type="password"
            ...
            <Form.Item
              label={<span className="text-sm font-medium text-gray-700">{t("descriptionOptional")}</span>}
              name="description"
            >
              <TextArea
                rows={3}
          ...
          <div className="flex justify-between items-center pt-6 border-t border-gray-100">
            <Tooltip title={t("getHelpTooltip")}>
