  ...
  return [
    {
      ...
      render: (_, tool) => {
        ...
        if (isFromConfig) {
          return <span className="text-xs">{t("noData")}</span>;
        }
      ...
      render: (_, tool) => {
        return (
          <span className="text-xs">
            {tool.created_at ? new Date(tool.created_at).toLocaleDateString() : t("noData")}
          </span>
        );
      },
    },
    {
      ...
      render: (_, tool) => {
        return (
          <span className="text-xs">
            {tool.updated_at ? new Date(tool.updated_at).toLocaleDateString() : t("noData")}
          </span>
        );
      },
    },
    {
      ...
      render: (_, tool) => {
        ...
        return <Tag color={isFromConfig ? "default" : "blue"}>{isFromConfig ? t("config") : t("db")}</Tag>;
      },
    },
    {
      title: t("actions"),
        ...
        return (
          <div className="flex items-center gap-2">
            <TableIconActionButton
              variant="Edit"
              tooltipText={t("editSearchTool")}
              disabled={isFromConfig}
              disabledTooltipText={t("configToolEditDisabled")}
              onClick={() => {
            ...
            <TableIconActionButton
              variant="Delete"
              tooltipText={t("deleteSearchTool")}
              disabled={isFromConfig}
              disabledTooltipText={t("configToolDeleteDisabled")}
              onClick={() => {
                if (toolId && !isFromConfig) {
                  onDelete(toolId);
                }
