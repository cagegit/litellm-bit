...
function ConfirmDialog({ action, serverName, isCurrentlyActive, onConfirm, onCancel }: ConfirmDialogProps) {
  ...
  const rejectBody = isCurrentlyActive ? t("rejectActiveBody") : t("rejectInactiveBody");
  return (
    <div className="fixed inset-0 bg-black/30 flex items-center justify-center z-50">
      <div className="bg-white rounded-xl shadow-xl p-6 max-w-sm w-full mx-4">
        ...
        <h3 className="text-base font-semibold text-gray-900 mb-1">
          {isApprove ? t("approveMcpServer") : t("rejectMcpServer")}
        </h3>
        <p className="text-sm text-gray-500 mb-4">
          {isApprove
            ? t("confirmApprove", { serverName })
            : t(isCurrentlyActive ? "confirmRejectActive" : "confirmRejectInactive", { serverName })}
        </p>
        {!isApprove && (
          <textarea
    ...
    <div className="mb-5 border border-gray-200 rounded-lg bg-white overflow-hidden">
      ...
      >
        <div className="flex items-center gap-2">
          ...
          <span className="text-sm font-semibold text-gray-800">{t("submissionRules")}</span>
          {activeLabels.length > 0 ? (
            <span className="text-xs text-gray-500">{t("requiredFields", { count: activeLabels.length })}</span>
          ) : (
            <span className="text-xs text-gray-400 italic">{t("noRulesSet")}</span>
          )}
        </div>
      ...
      {expanded && (
        <div className="border-t border-gray-100 px-4 pt-4 pb-4">
          <p className="text-xs text-gray-500 mb-4">{t("rulesDescription")}</p>
          ...
          <div className="flex-1 min-w-0">
            <div className="flex items-center gap-2 mb-1.5">
              ...
              >
                ...
                {t(approvalStatus === "pending_review" ? "pendingReview" : approvalStatus)}
              </span>
            </div>
            ...
            <div className="flex items-center gap-3 mt-1.5 text-xs text-gray-400">
              <span>
                {t("transport")} <span className="text-gray-600">{server.transport ?? "sse"}</span>
              </span>
              <span>·</span>
              <span>
                {t("submittedBy")} <span className="text-gray-600">{server.submitted_by ?? "—"}</span>
              </span>
              <span>·</span>
            ...
            {approvalStatus === "rejected" && server.review_notes && (
              <p className="text-xs text-red-600 mt-1.5">
                {t("rejectionReason")} {server.review_notes}
              </p>
            )}
          </div>
          ...
          >
            ...
            <div className="flex-1 min-w-0">
              <div className={`text-sm font-semibold leading-tight ${allPassed ? "text-green-800" : "text-red-800"}`}>
                {allPassed ? t("allChecksPassed") : t("checksFailed", { count: failCount })}
              </div>
              <div className="text-xs text-gray-500 mt-0.5">{t("passingFailing", { passCount, failCount })}</div>
            </div>
...
export function MCPSubmissionsTab({ accessToken }: MCPSubmissionsTabProps) {
  ...
  const handleSaveRules = async () => {
    ...
    try {
      ...
      NotificationsManager.success(t("rulesSaved"));
    } catch {
      NotificationsManager.fromBackend(t("failedToSaveRules"));
    } finally {
  ...
  async function handleApprove(serverId: string, serverName: string) {
    ...
    try {
      ...
      await fetchData();
      NotificationsManager.success(t("serverApproved", { serverName }));
    } catch {
      NotificationsManager.fromBackend(t("failedToApprove"));
    } finally {
  ...
  async function handleReject(serverId: string, serverName: string, reviewNotes?: string) {
    ...
    try {
      ...
      await fetchData();
      NotificationsManager.success(t("serverRejected", { serverName }));
    } catch {
      NotificationsManager.fromBackend(t("failedToReject"));
    } finally {
      setConfirmAction(null);
    }
  }
  ...
  return (
    <div className="p-6">
      ...
      <div className="grid grid-cols-4 gap-4 mb-6">
        <StatCard label={t("totalSubmitted")} value={summary.total} color="text-gray-900" />
        <StatCard label={t("pendingReview")} value={summary.pending_review} color="text-yellow-600" />
      ...
      <div className="space-y-3">
        ...
        {!isLoading && !error && filtered.length === 0 && (
          <div className="text-center py-12 text-gray-400 text-sm">{t("noMatch")}</div>
        )}
        {!isLoading &&
          !error &&
          filtered.map((server) => (
