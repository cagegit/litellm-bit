  ...
  return (
    ...
    >
        ...
        <div className="flex items-start gap-3">
          ...
          <div className="flex-1">
            ...
            <Text className="text-gray-400 text-xs block mt-1">
              {isEnabled ? t("usersCanCall") : t("usersCannotCall")}
            </Text>
          </div>
          <button
            type="button"
            ...
            title={t("editDisplayNameAndDescription")}
          >
            <EditOutlined />
          </button>
        </div>
      ...
      {isEditExpanded && (
        ...
        >
          <div>
            <Text className="text-xs font-medium text-gray-600 mb-1 block">{t("displayName")}</Text>
            <Input
              placeholder={tool.name}
              value={toolNameToDisplayName[tool.name] || ""}
              onChange={(e) => onDisplayNameChange(tool.name, e.target.value)}
            />
            <Text className="text-xs text-gray-400 mt-1 block">{t("displayNameHelp")}</Text>
          </div>
          <div>
            <Text className="text-xs font-medium text-gray-600 mb-1 block">{t("description")}</Text>
            <Input.TextArea
              placeholder={tool.description || t("noDescription")}
              value={toolNameToDescription[tool.name] || ""}
              onChange={(e) => onDescriptionChange(tool.name, e.target.value)}
              rows={2}
            />
            <Text className="text-xs text-gray-400 mt-1 block">{t("descriptionHelp")}</Text>
          </div>
        </div>
      )}
    </div>
      ...
      <div className="space-y-4">
        <div className="flex items-center justify-between">
          <div className="flex items-center gap-2">
            ...
            <Title>{t("toolConfiguration")}</Title>
            {tools.length > 0 && (
              <Badge
                count={tools.length}
                style={{
          ...
          {tools.length > 0 && (
            <Radio.Group
              ...
              buttonStyle="solid"
              options={[
                { label: t("riskGroups"), value: "crud" },
                { label: t("flatList"), value: "flat" },
              ]}
            />
          )}
        </div>
        ...
        <div className="bg-blue-50 border border-blue-200 rounded-lg p-3">
          <Text className="text-blue-800 text-sm">
            <strong>{t("selectToolsLabel")}</strong>
            {t("selectToolsDescription")}
          </Text>
        </div>
        ...
        {isLoadingTools && (
          <div className="flex items-center justify-center py-6">
            <Spin size="large" />
            <Text className="ml-3">{t("loadingTools")}</Text>
          </div>
        )}
        ...
        {!isLoadingTools &&
          ...
          (keyTools && keyTools.length > 0 ? (
            <div className="text-center py-4 text-gray-400 border rounded-lg border-dashed">
              <ToolOutlined className="text-2xl mb-2" />
              <Text>{t("noToolsLoaded")}</Text>
              <Text className="text-sm block mt-1">
                {t("expectedTools", { tools: keyTools.map((t) => t.name).join(", ") })}
              </Text>
            </div>
          ) : (
            <div className="text-center py-6 text-gray-400 border rounded-lg border-dashed">
              <ToolOutlined className="text-2xl mb-2" />
              <Text>{t("noToolsAvailable")}</Text>
              <br />
              <Text className="text-sm">{t("noToolsAvailableHelp")}</Text>
            </div>
          ))}
        ...
        {!canFetchTools && (formValues.url || formValues.spec_path) && (
          <div className="text-center py-6 text-gray-400 border rounded-lg border-dashed">
            <ToolOutlined className="text-2xl mb-2" />
            <Text>{t("completeFields")}</Text>
            <br />
            <Text className="text-sm">{t("completeFieldsHelp")}</Text>
          </div>
        )}
          ...
          <div className="space-y-3">
            <div className="flex items-center gap-2 p-3 bg-green-50 rounded-lg border border-green-200">
              ...
              <Text className="text-green-700 font-medium">
                {t("toolsEnabledCount", { enabled: effectiveAllowedTools.length, total: tools.length })}
              </Text>
            </div>
              ...
              <>
                {filteredTools.length === 0 ? (
                  <div className="text-center py-6 text-gray-400 border rounded-lg border-dashed">
                    ...
                    <Text>{t("noToolsFound", { term: toolSearchTerm })}</Text>
                  </div>
                ) : (
                    ...
                    {restFiltered.length > 0 && (
                      <>
                        <div className="flex items-center justify-between px-1 pt-2">
                          <p className="text-xs font-semibold text-gray-500 uppercase tracking-wide">
                            {pinnedFiltered.length > 0 ? t("allTools") : t("tools")}
                          </p>
                          <div className="flex gap-2">
                            <button
                              type="button"
