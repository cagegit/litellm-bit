...
const UserEnvVarsModal: React.FC<UserEnvVarsModalProps> = ({ server, open, accessToken, onClose, onSaved }) => {
  ...
  const saveMutation = useMutation({
    ...
    onSuccess: (saved) => {
      NotificationsManager.success(t("credentialsSaved"));
      onSaved?.(saved);
      onClose();
    },
    onError: (err) => {
      NotificationsManager.fromBackend(
        t("failedToSaveEnvVars", { error: err instanceof Error ? err.message : String(err) }),
      );
    },
  });
  ...
  const displayName = server?.server_name || server?.alias || server?.server_id || t("mcpServer");
        ...
        <div>
          <div className="flex items-center gap-2">
            <Title level={5} style={{ margin: 0 }}>
              {t("setYourCredentials")}
            </Title>
            <Tag color="blue">{t("perUser")}</Tag>
          </div>
    ...
    >
      <div className="space-y-4 mt-2">
        {isLoading ? (
          ...
          <Alert type="error" showIcon message={t("failedToLoadEnvVars")} />
        ) : required.length === 0 ? (
          <Alert type="info" showIcon message={t("noPerUserFields")} />
        ) : (
          <>
            <Text className="text-sm text-gray-600 block">{t("credentialsDescription")}</Text>
                ...
                <Form.Item
                  ...
                  label={
                    <span className="flex items-center gap-2">
                      ...
                      {spec.is_set && <Tag color="green">{t("set")}</Tag>}
                    </span>
                  }
                  extra={spec.description || undefined}
                  rules={
                    spec.is_set ? undefined : [{ required: true, message: t("fieldRequired", { name: spec.name }) }]
                  }
                >
                  <Input.Password
                    placeholder={
                      spec.is_set ? t("enterNewValue") : spec.description || t("enterYour", { name: spec.name })
                    }
                    visibilityToggle
                  />
                </Form.Item>
