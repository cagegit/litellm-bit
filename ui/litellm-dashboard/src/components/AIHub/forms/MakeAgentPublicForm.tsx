...
const MakeAgentPublicForm: React.FC<MakeAgentPublicFormProps> = ({
  ...
  const handleNext = () => {
    if (currentStep === 0) {
      if (selectedAgents.size === 0) {
        NotificationsManager.fromBackend(t("pleaseSelectAtLeastOneAgent"));
        return;
      }
      setCurrentStep(1);
    }
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      NotificationsManager.success(t("successfullyMadePublic", { count: agentIdsToMakePublic.length }));
      handleClose();
      onSuccess();
    } catch (error) {
      console.error("Error making agents public:", error);
      NotificationsManager.fromBackend(t("failedToMakePublic"));
    } finally {
      setLoading(false);
    }
  };
    ...
    return (
      <div className="space-y-4">
        <div className="flex items-center justify-between">
          <Title>{t("selectAgentsToMakePublic")}</Title>
          <div className="flex items-center space-x-2">
            ...
            >
              {t("selectAll")} {agentHubData.length > 0 && `(${agentHubData.length})`}
            </Checkbox>
          </div>
        </div>

        <Text className="text-sm text-gray-600">{t("selectAgentsDescription")}</Text>

        <div className="max-h-96 overflow-y-auto border rounded-lg p-4">
          <div className="space-y-3">
            {agentHubData.length === 0 ? (
              <div className="text-center py-8 text-gray-500">
                <Text>{t("noAgentsAvailable")}</Text>
              </div>
            ) : (
                        ...
                        <div className="flex flex-wrap gap-1 mt-1">
                          ...
                          {agent.skills.length > 3 && (
                            <Text className="text-xs text-gray-500">
                              {t("moreCount", { count: agent.skills.length - 3 })}
                            </Text>
                          )}
                        </div>
                      )}
        ...
        {selectedAgents.size > 0 && (
          <div className="bg-blue-50 border border-blue-200 rounded-lg p-3">
            <Text className="text-sm text-blue-800">
              <strong>{selectedAgents.size}</strong> {t("agentsSelected", { count: selectedAgents.size })}
            </Text>
          </div>
        )}
      </div>
  ...
  const renderStep2Content = () => {
    return (
      <div className="space-y-4">
        <Title>{t("confirmMakingAgentsPublic")}</Title>

        <div className="bg-yellow-50 border border-yellow-200 rounded-lg p-4">
          <Text className="text-sm text-yellow-800">
            <strong>{t("warning")}:</strong> {t("makingPublicWarning", { path: "/ui/model_hub_table" })}
          </Text>
        </div>

        <div className="space-y-3">
          <Text className="font-medium">{t("agentsToBeMadePublic")}</Text>
        ...
        <div className="bg-blue-50 border border-blue-200 rounded-lg p-3">
          <Text className="text-sm text-blue-800">
            {t("total")}: <strong>{selectedAgents.size}</strong> {t("willBeMadePublic", { count: selectedAgents.size })}
          </Text>
        </div>
      </div>
    );
