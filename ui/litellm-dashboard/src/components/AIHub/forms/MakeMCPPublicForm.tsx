...
const MakeMCPPublicForm: React.FC<MakeMCPPublicFormProps> = ({
  ...
  const handleNext = () => {
    if (currentStep === 0) {
      if (selectedServers.size === 0) {
        NotificationsManager.fromBackend(t("pleaseSelectServer"));
        return;
      }
      setCurrentStep(1);
    }
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      NotificationsManager.success(t("successfullyMadePublic", { count: serverIdsToMakePublic.length }));
      handleClose();
      onSuccess();
    } catch (error) {
      console.error("Error making MCP servers public:", error);
      NotificationsManager.fromBackend(t("failedMakePublic"));
    } finally {
      setLoading(false);
    }
  };
    ...
    return (
      <div className="space-y-4">
        <div className="flex items-center justify-between">
          <Title>{t("selectServersTitle")}</Title>
          <div className="flex items-center space-x-2">
            ...
            >
              {t("selectAll")} {mcpHubData.length > 0 && `(${mcpHubData.length})`}
            </Checkbox>
          </div>
        </div>

        <Text className="text-sm text-gray-600">{t("selectDescription")}</Text>

        <div className="max-h-96 overflow-y-auto border rounded-lg p-4">
          <div className="space-y-3">
            {mcpHubData.length === 0 ? (
              <div className="text-center py-8 text-gray-500">
                <Text>{t("noServersAvailable")}</Text>
              </div>
            ) : (
                        ...
                        <div className="flex flex-wrap gap-1 mt-1">
                          ...
                          {server.allowed_tools.length > 3 && (
                            <Text className="text-xs text-gray-500">
                              +{server.allowed_tools.length - 3} {t("more")}
                            </Text>
                          )}
                        </div>
                      )}
        ...
        {selectedServers.size > 0 && (
          <div className="bg-blue-50 border border-blue-200 rounded-lg p-3">
            <Text className="text-sm text-blue-800">
              <strong>{selectedServers.size}</strong> {t("serversSelected", { count: selectedServers.size })}
            </Text>
          </div>
        )}
      </div>
      ...
      <div className="space-y-4">
        ...
        <div className="bg-yellow-50 border border-yellow-200 rounded-lg p-4">
          <Text className="text-sm text-yellow-800">
            <strong>{t("warning")}:</strong> {t("warningPublicPrefix")} <code>/ui/model_hub_table</code>{" "}
            {t("warningPublicSuffix")}
          </Text>
        </div>

        <div className="space-y-3">
          <Text className="font-medium">{t("serversToMakePublic")}</Text>
        ...
        <div className="bg-blue-50 border border-blue-200 rounded-lg p-3">
          <Text className="text-sm text-blue-800">
            {t("total")}: <strong>{selectedServers.size}</strong>{" "}
            {t("willBeMadePublic", { count: selectedServers.size })}
          </Text>
        </div>
      </div>
    );
