...
const UsefulLinksManagement: React.FC<UsefulLinksManagementProps> = ({ accessToken, userRole }) => {
  ...
  const saveLinksToBackend = async (updatedLinks: Link[]) => {
    ...
    try {
      ...
      NotificationsManager.fromBackend(`${t("failedToSaveLinks")} - ${error}`);
      return false;
    }
  };
  ...
  const handleAddLink = async () => {
    ...
    try {
      ...
      NotificationsManager.fromBackend(t("pleaseEnterValidUrl"));
      return;
    }

    // Check for duplicate display names
    if (links.some((link) => link.displayName === newLink.displayName)) {
      NotificationsManager.fromBackend(t("linkWithDisplayNameExists"));
      return;
    }
    ...
    if (await saveLinksToBackend(updatedLinks)) {
      ...
      NotificationsManager.success(t("linkAddedSuccessfully"));
    }
  };
  ...
  const handleUpdateLink = async () => {
    ...
    if (await saveLinksToBackend(updatedLinks)) {
      setLinks(updatedLinks);
      setEditingLink(null);
      NotificationsManager.success(t("linkUpdatedSuccessfully"));
    }
  };
  ...
  const deleteLink = async (linkId: string) => {
    ...
    if (await saveLinksToBackend(updatedLinks)) {
      setLinks(updatedLinks);
      NotificationsManager.success(t("linkDeletedSuccessfully"));
    }
  };
  ...
  const handleSaveRearranging = async () => {
    if (await saveLinksToBackend(links)) {
      setIsRearranging(false);
      setOriginalLinksOrder([]);
      NotificationsManager.success(t("linkOrderSavedSuccessfully"));
    }
  };
    ...
    <Card className="mb-6">
      <div className="flex items-center justify-between cursor-pointer" onClick={() => setIsExpanded(!isExpanded)}>
        <div className="flex flex-col">
          <Title className="mb-0">{t("linkManagement")}</Title>
          <p className="text-sm text-gray-500">{t("manageLinksDescription")}</p>
        </div>
      ...
      {isExpanded && (
        <div className="mt-4">
          <div className="mb-6">
            <Text className="text-sm font-medium text-gray-700 mb-2">{t("addNewLink")}</Text>
            <div className="grid grid-cols-3 gap-4">
              <div>
                ...
                <input
                  ...
                  placeholder={t("friendlyName")}
                  className="w-full px-3 py-2 border border-gray-300 rounded-md text-sm"
                />
              </div>
              <div>
                <label className="block text-xs text-gray-500 mb-1">{t("url")}</label>
                <input
                  type="text"
                  value={newLink.url}
                  onChange={(e) =>
          ...
          <div className="flex items-center justify-between mb-2">
            <Text className="text-sm font-medium text-gray-700">{t("manageExistingLinks")}</Text>
          ...
          <div className="rounded-lg custom-border relative">
            <div className="overflow-x-auto">
              <Table className="[&_td]:py-0.5 [&_th]:py-1">
                <TableHead>
                  <TableRow>
                    <TableHeaderCell className="py-1 h-8">{t("displayName")}</TableHeaderCell>
                    <TableHeaderCell className="py-1 h-8">{t("url")}</TableHeaderCell>
                            ...
                            {isRearranging ? (
                              <div className="flex space-x-2">
                                <TableIconActionButton
                                  ...
                                  tooltipText={t("moveUp")}
                                  disabled={index === 0}
                                  disabledTooltipText={t("alreadyAtTheTop")}
                                  dataTestId={`move-up-${link.id}`}
                                />
                                <TableIconActionButton
                                  variant="Down"
                                  onClick={() => handleMoveDown(index)}
                                  tooltipText={t("moveDown")}
                                  disabled={index === links.length - 1}
                                  disabledTooltipText={t("alreadyAtTheBottom")}
                              ...
                              <div className="flex space-x-2">
                                <TableIconActionButton
                                  ...
                                  tooltipText={t("openLink")}
                                  dataTestId={`open-link-${link.id}`}
                                />
                                <TableIconActionButton
                                  variant="Edit"
                                  onClick={() => handleEditLink(link)}
                                  tooltipText={t("editLink")}
                                  dataTestId={`edit-link-${link.id}`}
                                />
                                <TableIconActionButton
                                  variant="Delete"
                                  onClick={() => deleteLink(link.id)}
                                  tooltipText={t("deleteLink")}
