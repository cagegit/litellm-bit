...
const EditUserModal: React.FC<EditUserModalProps> = ({ visible, possibleUIRoles, onCancel, user, onSubmit }) => {
  ...
  return (
    <Modal open={visible} onCancel={handleCancel} footer={null} title={t("editUser") + " " + user.user_id} width={1000}>
      <Form
        form={form}
      ...
      >
        <>
          <Form.Item className="mt-8" label={t("userEmail")} tooltip={t("emailOfTheUser")} name="user_email">
            <TextInput />
          </Form.Item>

          <Form.Item label={t("userId")} name="user_id" hidden={true}>
            <TextInput />
          </Form.Item>
          ...
          <Form.Item label={t("spendUsd")} name="spend" tooltip={t("spendTooltip")} help={t("spendHelp")}>
            <InputNumber min={0} step={0.01} />
          </Form.Item>

          <Form.Item
            label={t("userBudgetUsd")}
            name="max_budget"
            tooltip={t("maxBudgetTooltip")}
            help={t("maxBudgetHelp")}
          >
