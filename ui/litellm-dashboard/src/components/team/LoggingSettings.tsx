    ...
    return (
      <div className="mt-6 pt-4 border-t border-gray-100">
        <div className="flex items-center space-x-2 mb-4">
          ...
          <span className="text-sm font-medium text-gray-700">{t("integrationParameters")}</span>
        </div>
          ...
          {Object.entries(dynamicParams).map(([paramName, paramType]) => (
            <div key={paramName} className="space-y-2">
              <label className="text-sm font-medium text-gray-700 capitalize flex items-center space-x-1">
                ...
                <Tooltip title={t("envVarReference", { name: paramName.toUpperCase() })}>
              ...
              {paramType === "number" && <span className="text-xs text-gray-500">{t("valueMustBeBetween0And1")}</span>}
              {paramType === "number" ? (
                <NumericalInput
                  step={0.01}
                  width={400}
                  placeholder={t("osEnvPlaceholder", { name: paramName.toUpperCase() })}
    ...
    <div className="space-y-6">
      ...
      <div className="space-y-4">
        <div className="flex items-center space-x-2">
          ...
          <span className="text-base font-semibold text-gray-800">{t("disabledCallbacks")}</span>
          <Tooltip title={t("disabledCallbacksTooltip")}>
            <InfoCircleOutlined className="text-gray-400 cursor-help" />
          </Tooltip>
        </div>

        <div className="space-y-2">
          <label className="text-sm font-medium text-gray-700">{t("disabledCallbacks")}</label>
          <Select
            mode="multiple"
            placeholder={t("selectCallbacksToDisable")}
          ...
          <div className="text-xs text-gray-500">{t("disabledCallbacksDescription")}</div>
        </div>
      </div>
      ...
      <div className="flex justify-between items-center">
        <div className="flex items-center space-x-2">
          ...
          <span className="text-base font-semibold text-gray-800">{t("loggingIntegrations")}</span>
          <Tooltip title={t("loggingIntegrationsTooltip")}>
            <InfoCircleOutlined className="text-gray-400 cursor-help" />
          </Tooltip>
        </div>
        <Button
            ...
            >
              ...
              <div className="space-y-4">
                <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                  <div className="space-y-2">
                    <label className="text-sm font-medium text-gray-700">{t("integrationType")}</label>
                    <Select
                  ...
                  <div className="space-y-2">
                    <label className="text-sm font-medium text-gray-700">{t("eventType")}</label>
                    <Select
      ...
      {value.length === 0 && (
        <div className="text-center py-12 text-gray-500 border-2 border-dashed border-gray-200 rounded-lg bg-gray-50/50">
          ...
          <div className="text-base font-medium mb-1">{t("noLoggingIntegrations")}</div>
          <div className="text-sm text-gray-400">{t("noLoggingIntegrationsDesc")}</div>
        </div>
      )}
    </div>
  );
