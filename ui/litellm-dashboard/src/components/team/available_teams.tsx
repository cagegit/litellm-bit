...
const AvailableTeamsPanel: React.FC<AvailableTeamsProps> = ({ accessToken, userID }) => {
  ...
  const handleJoinTeam = async (teamId: string) => {
    ...
    try {
      ...
      NotificationsManager.success(t("successfullyJoinedTeam"));
      // Update available teams list
      setAvailableTeams((teams) => teams.filter((team) => team.team_id !== teamId));
    } catch (error) {
      console.error("Error joining team:", error);
      NotificationsManager.fromBackend(t("failedToJoinTeam"));
    }
  };
        ...
        <TableBody>
          {availableTeams.map((team) => (
            <TableRow key={team.team_id}>
              ...
              <TableCell>
                <Text>{team.description || t("noDescription")}</Text>
              </TableCell>
              <TableCell>
                <Text>{t("membersCount", { count: team.members_with_roles.length })}</Text>
              </TableCell>
              <TableCell>
