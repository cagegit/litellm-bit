...
export function LoggingSettingsView({
  ...
  const getEventTypeLabel = (eventType: string) => {
    switch (eventType) {
      case "success":
        return t("successOnly");
      case "failure":
        return t("failureOnly");
      case "success_and_failure":
    ...
    <div className="space-y-6">
      ...
      <div className="space-y-3">
        <div className="flex items-center gap-2">
          ...
          <span className="font-semibold text-gray-900">{t("loggingIntegrations")}</span>
        ...
        {loggingConfigs.length > 0 ? (
                  ...
                  <div className="flex items-center gap-3">
                    ...
                    <div>
                      ...
                      <span className="block text-xs text-blue-600">
                        {t("parametersConfigured", { count: Object.keys(config.callback_vars).length })}
                      </span>
                    </div>
                  </div>
          ...
          <div className="flex items-center gap-2 px-3 py-2 rounded-lg bg-gray-50 border border-gray-200">
            ...
            <span className="text-gray-500 text-sm">{t("noLoggingIntegrationsConfigured")}</span>
          </div>
        )}
      </div>
      ...
      <div className="space-y-3">
        <div className="flex items-center gap-2">
          ...
          <span className="font-semibold text-gray-900">{t("disabledCallbacks")}</span>
        ...
        {disabledCallbacks.length > 0 ? (
                ...
                >
                  <div className="flex items-center gap-3">
                    ...
                    <div>
                      ...
                      <span className="block text-xs text-red-600">{t("disabledForThisKey")}</span>
                    </div>
                  </div>
          ...
          <div className="flex items-center gap-2 px-3 py-2 rounded-lg bg-gray-50 border border-gray-200">
            ...
            <span className="text-gray-500 text-sm">{t("noCallbacksDisabled")}</span>
          </div>
        )}
      </div>
    </div>
    ...
    return (
      <div className={`bg-white border border-gray-200 rounded-lg p-6 ${className}`}>
        <div className="flex items-center gap-2 mb-6">
          <div>
            <span className="block font-semibold text-gray-900">{t("loggingSettings")}</span>
            <span className="block text-xs text-gray-500">{t("loggingSettingsDescription")}</span>
          </div>
        </div>
        {content}
      </div>
  ...
  return (
    <div className={`${className}`}>
      <span className="block font-medium text-gray-900 mb-3">{t("loggingSettings")}</span>
      {content}
    </div>
  );
}
