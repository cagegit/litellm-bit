  ...
  return (
    <div className="mt-4">
      <div className="flex items-center mb-2 cursor-pointer" onClick={() => setEntityListExpanded(!entityListExpanded)}>
        ...
        <h4 className="font-medium">{t("detectedEntities", { count: entities.length })}</h4>
      </div>
                ...
                >
                  <div className="flex items-center">
                    ...
                    <span className={`font-mono ${getScoreColor(entity.score)}`}>
                      {t("score", { score: entity.score.toFixed(2) })}
                    </span>
                  </div>
                  <span className="text-xs text-gray-500">
                    {t("position", { start: entity.start, end: entity.end })}
                  </span>
                </div>
                ...
                {isExpanded && (
                  <div className="p-3 border-t bg-white">
                    <div className="grid grid-cols-2 gap-4 mb-2">
                      <div className="space-y-2">
                        <div className="flex">
                          <span className="font-medium w-1/3">{t("entityType")}</span>
                          <span>{entity.entity_type}</span>
                        </div>
                        <div className="flex">
                          <span className="font-medium w-1/3">{t("positionLabel")}</span>
                          <span>{t("characters", { start: entity.start, end: entity.end })}</span>
                        </div>
                        <div className="flex">
