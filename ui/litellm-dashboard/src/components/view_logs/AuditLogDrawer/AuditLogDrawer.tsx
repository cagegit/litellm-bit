...
function DiffSection({ log }: { log: AuditLogEntry }) {
  ...
  if (isUpdateAction && before_value && updated_values) {
    ...
    displayBefore = Object.keys(changedBefore).length > 0 ? changedBefore : { note: t("noDifferingFieldsDetected") };
    displayAfter = Object.keys(changedAfter).length > 0 ? changedAfter : { note: t("noDifferingFieldsDetected") };
  }
    ...
    if (!value || Object.keys(value).length === 0) {
      return (
        <div className="bg-white rounded border overflow-hidden">
          ...
          <p className="px-3 py-3 text-xs text-gray-400 italic m-0">{t("na")}</p>
        </div>
      );
    }
          ...
          <div className="bg-white rounded border overflow-hidden">
            ...
            <div className="px-3 py-3 space-y-1 text-xs">
              {value.token !== undefined && (
                <p>
                  <span className="text-gray-500">{t("token")}:</span> {value.token ?? t("na")}
                </p>
              )}
              {value.spend !== undefined && (
                <p>
                  <span className="text-gray-500">{t("spend")}:</span> ${Number(value.spend).toFixed(6)}
                </p>
              )}
              {value.max_budget !== undefined && (
                <p>
                  <span className="text-gray-500">{t("maxBudget")}:</span> ${Number(value.max_budget).toFixed(6)}
                </p>
              )}
            </div>
          </div>
...
export function AuditLogDrawer({ open, onClose, log }: AuditLogDrawerProps) {
  ...
  const tableNameKey = TABLE_NAME_DISPLAY[log.table_name];
  const tableDisplay = tableNameKey ? t(tableNameKey) : log.table_name;
    ...
    >
      ...
      <div className="px-6 py-5">
        ...
        <div className="bg-gray-50 border rounded-lg p-4 mb-5">
          <p className="text-xs font-semibold text-gray-700 mb-2 uppercase tracking-wide">{t("details")}</p>
          ...
          />
          <MetadataRow label={t("changedBy")} value={<DefaultProxyAdminTag userId={log.changed_by} />} />
          <MetadataRow
            label={t("apiKeyHash")}
            value={
