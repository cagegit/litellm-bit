...
function EvalEntryCard({ entry }: { entry: EvalInformation }) {
  ...
  const columns = [
    ...
    {
      title: (
        <Tooltip title={t("weightedTooltip")}>
    ...
    <Card
      ...
      title={
        <Space>
          ...
          <Tag color={passed ? "success" : "error"}>{passed ? t("passed") : t("failed")}</Tag>
          <Tooltip title={t("overallScoreTooltip")}>
            <Text type="secondary" style={{ fontSize: 12, cursor: "help", borderBottom: "1px dashed #aaa" }}>
              {t("scoreOutOf", { score: entry.overall_score?.toFixed(0) })}
              {entry.threshold != null && t("threshold", { value: entry.threshold })}
            </Text>
          </Tooltip>
        </Space>
      }
      extra={
        <Space size="small">
          {entry.judge_model && (
            <Text type="secondary" style={{ fontSize: 12 }}>
              {t("judge", { model: entry.judge_model })}
            </Text>
          )}
          {entry.iteration != null && (
            <Text type="secondary" style={{ fontSize: 12 }}>
              {t("iteration", { num: entry.iteration + 1 })}
            </Text>
          )}
        </Space>
      }
    >
      {entry.eval_error && (
        <Text type="warning" style={{ display: "block", marginBottom: 8, fontSize: 12 }}>
          {t("judgeError", { error: entry.eval_error })}
        </Text>
      )}
      ...
      {verdicts.length > 0 ? (
        ...
        <Text type="secondary" style={{ fontSize: 12 }}>
          {t("noBreakdown", { score: entry.overall_score?.toFixed(1) })}
        </Text>
      )}
    </Card>
  );
