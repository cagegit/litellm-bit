        ...
        return (
            ...
            rules={
              isRequired
                ? [
                    {
                      required: true,
                      message: `${t("pleaseEnter")} ${fieldLabel.toLowerCase()}`,
                    },
                  ]
                : undefined
            }
          >
            {paramType === "password" ? (
              <Input.Password
                size="large"
                placeholder={`${t("enterYour")} ${fieldLabel.toLowerCase()}`}
              ...
              <Input
                type="number"
                size="large"
                placeholder={`${t("enter")} ${fieldLabel.toLowerCase()}`}
                className="w-full rounded-md border-gray-300 shadow-sm focus:border-blue-500 focus:ring-blue-500"
                min={0}
                max={1}
                step={0.1}
...
const CallbackSelector: React.FC<CallbackSelectorProps> = ({
  ...
  return (
    <FormItem
      label={t("callback")}
      name="callback"
      rules={disabled ? undefined : [{ required: true, message: t("pleaseSelectCallback") }]}
    >
      <Select
        placeholder={t("chooseLoggingCallback")}
        size="large"
        className="w-full"
        showSearch
        disabled={disabled}
            ...
            <SelectItem key={callbackConfig.id} value={callbackConfig.id}>
              <div className="flex items-center space-x-3 py-1">
                <div className="w-6 h-6 flex items-center justify-center">
                  ...
                  <img
                    src={logoSrc}
                    alt={`${callbackConfig.displayName} ${t("logo")}`}
...
const Settings: React.FC<SettingsPageProps> = ({ accessToken, userRole, userID, premiumUser }) => {
  ...
  useEffect(() => {
    ...
    getCallbackConfigsCall(accessToken)
      ...
      .catch((error) => {
        NotificationsManager.fromBackend(t("failedToLoadCallbackConfigs") + parseErrorMessage(error));
      });
  }, [accessToken]);
  ...
  const alerts_to_UI_NAME: Record<string, string> = {
    llm_exceptions: "llmExceptions",
    llm_too_slow: "llmResponsesTooSlow",
    llm_requests_hanging: "llmRequestsHanging",
    budget_alerts: "budgetAlerts",
    db_exceptions: "databaseExceptions",
    daily_reports: "spendReports",
    outage_alerts: "outageAlerts",
    region_outage_alerts: "regionOutageAlerts",
  };
  ...
  const handleCallbackSubmit = async (formValues: Record<string, any>, callbackName: string, isEdit: boolean) => {
    ...
    try {
      ...
      NotificationsManager.success(isEdit ? t("callbackUpdated") : t("callbackAdded", { name: callbackName }));
  ...
  const handleSaveAlerts = async () => {
    ...
    NotificationsManager.success(t("alertsUpdated"));
  };
  const handleSaveChanges = (callback: any) => {
    ...
    NotificationsManager.success(t("callbackUpdated"));
  };
  ...
  const confirmDeleteCallback = async () => {
    ...
    try {
      ...
      NotificationsManager.success(t("callbackDeleted", { name: callbackToDelete.name }));
  ...
  return (
    <div className="w-full mx-4">
      <Grid numItems={1} className="gap-2 p-8 w-full mt-2">
        <TabGroup>
          <TabList variant="line" defaultValue="1">
            <Tab value="1">{t("loggingCallbacks")}</Tab>
            <Tab value="2">{t("cloudZeroCostTracking")}</Tab>
            <Tab value="2">{t("alertingTypes")}</Tab>
            <Tab value="3">{t("alertingSettings")}</Tab>
            <Tab value="4">{t("emailAlerts")}</Tab>
          </TabList>
          <TabPanels>
            <TabPanel>
              <LoggingCallbacksTable
                ...
                onTest={async (cb) => {
                  try {
                    ...
                    NotificationsManager.success(t("healthCheckTriggered"));
                  } catch (error) {
                ...
                <Table>
                  <TableHead>
                    <TableRow>
                      ...
                      <TableHeaderCell></TableHeaderCell>
                      <TableHeaderCell>{t("slackWebhookUrl")}</TableHeaderCell>
                    </TableRow>
                  </TableHead>
                ...
                <Button
                  onClick={async () => {
                    try {
                      ...
                      NotificationsManager.success(t("alertTestTriggered"));
                    } catch (error) {
                      NotificationsManager.fromBackend(parseErrorMessage(error));
                    }
                  }}
        ...
        >
          ...
          <div className="flex justify-end space-x-3 pt-6 mt-6 border-t border-gray-200">
            ...
            <Button2 htmlType="submit" loading={isAddingCallback} disabled={isAddingCallback}>
              {isAddingCallback ? t("adding") : t("addCallback")}
            </Button2>
          </div>
        </Form>
      </Modal>
        ...
        >
          ...
          <div className="flex justify-end space-x-3 pt-6 mt-6 border-t border-gray-200">
            ...
            <Button2
              ...
              disabled={isUpdatingCallback}
            >
              {isUpdatingCallback ? t("saving") : t("saveChanges")}
            </Button2>
          </div>
        </Form>
      </Modal>
      ...
      <DeleteResourceModal
        ...
        title={t("deleteCallback")}
        message={t("confirmDeleteCallbackMessage")}
        resourceInformationTitle={t("callbackInformation")}
        resourceInformation={[
          { label: t("callbackName"), value: callbackToDelete?.name },
          { label: t("mode"), value: callbackToDelete?.mode || t("success") },
        ]}
        onCancel={() => {
          setShowDeleteConfirmModal(false);
          setCallbackToDelete(null);
