...
const ContentFilterConfiguration: React.FC<ContentFilterConfigurationProps> = ({
  ...
  const handleAddPrebuiltPattern = () => {
    if (!selectedPatternName) {
      NotificationsManager.error(t("pleaseSelectPattern"));
      return;
    }
  ...
  const handleAddCustomPattern = () => {
    if (!customPatternName || !customPatternRegex) {
      NotificationsManager.error(t("pleaseProvidePatternNameAndRegex"));
      return;
    }
  ...
  const handleAddKeyword = () => {
    if (!newKeyword) {
      NotificationsManager.error(t("pleaseEnterKeyword"));
      return;
    }
  ...
  return (
    <div className="space-y-6">
      {!showStep && (
        <div>
          <Text type="secondary">{t("configurePatternsKeywordsAndContentCategoriesDescription")}</Text>
        </div>
      )}
          ...
          title={
            <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center" }}>
              ...
              <Text type="secondary" style={{ fontSize: 14, fontWeight: 400 }}>
                {t("detectSensitiveInformationUsingRegexPatternsSSNCreditCardsAPIKeysEtc")}
              </Text>
            </div>
          }
          size="small"
        ...
        <Card
          title={
            <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center" }}>
              ...
              <Text type="secondary" style={{ fontSize: 14, fontWeight: 400 }}>
                {t("blockOrMaskSpecificSensitiveTermsAndPhrases")}
              </Text>
            </div>
          }
          size="small"
