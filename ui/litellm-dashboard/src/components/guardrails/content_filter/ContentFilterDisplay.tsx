  ...
  return (
    <>
      {categories.length > 0 && (
        <Card className="mt-6">
          <div className="flex justify-between items-center mb-4">
            <Text className="text-lg font-semibold">{t("contentCategories")}</Text>
            <Badge color="blue">{t("categoriesConfigured", { count: categories.length })}</Badge>
          </div>
          <CategoryTable
      ...
      {patterns.length > 0 && (
        <Card className="mt-6">
          <div className="flex justify-between items-center mb-4">
            <Text className="text-lg font-semibold">{t("patternDetection")}</Text>
            <Badge color="blue">{t("patternsConfigured", { count: patterns.length })}</Badge>
          </div>
          <PatternTable
            patterns={patterns}
      ...
      {blockedWords.length > 0 && (
        <Card className="mt-6">
          <div className="flex justify-between items-center mb-4">
            <Text className="text-lg font-semibold">{t("blockedKeywords")}</Text>
            <Badge color="blue">{t("keywordsConfigured", { count: blockedWords.length })}</Badge>
          </div>
          <KeywordTable
