...
export function GuardrailTestPanel({
  ...
  const handleSubmit = () => {
    if (!inputText.trim()) {
      NotificationsManager.fromBackend(t("pleaseEnterText"));
      return;
    }
  ...
  const handleCopyInput = async () => {
    ...
    if (success) {
      NotificationsManager.success(t("inputCopied"));
    } else {
      NotificationsManager.fromBackend(t("failedToCopy"));
    }
  };
        ...
        <div className="flex items-center space-x-3">
          <div className="flex-1 min-w-0">
            <div className="flex items-center space-x-2 mb-1">
              <h2 className="text-lg font-semibold text-gray-900">{t("testGuardrails")}</h2>
            ...
            <p className="text-sm text-gray-500">{t("testGuardrailsDescription", { count: guardrailNames.length })}</p>
          </div>
        </div>
      </div>
        ...
        <div className="space-y-3">
          <div>
            <div className="flex justify-between items-center mb-2">
              <div className="flex items-center gap-2">
                ...
                <Tooltip title={t("pressEnterToSubmit")}>
          ...
          <div className="pt-2">
            <Button onClick={handleSubmit} loading={isLoading} disabled={!inputText.trim()} className="w-full">
              {isLoading
                ? t("testingGuardrails", { count: guardrailNames.length })
                : t("testGuardrailsButton", { count: guardrailNames.length })}
            </Button>
          </div>
        </div>
