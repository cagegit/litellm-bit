...
const GuardrailTestPlayground: React.FC<GuardrailTestPlaygroundProps> = ({
  ...
  const handleTestGuardrails = async (text: string) => {
    ...
    if (results.length > 0) {
      NotificationsManager.success(t("guardrailsAppliedSuccessfully", { count: results.length }));
    }
    if (errors.length > 0) {
      NotificationsManager.fromBackend(t("guardrailsFailed", { count: errors.length }));
    }
  };
          ...
          <div className="w-1/4 border-r border-gray-200 flex flex-col overflow-hidden">
            ...
            <div className="flex-1 overflow-auto">
              {isLoading ? (
                ...
                <div className="p-4">
                  <Empty description={searchQuery ? t("noGuardrailsMatchSearch") : t("noGuardrailsAvailable")} />
                </div>
              ) : (
                <List
            ...
            <div className="p-3 border-t border-gray-200 bg-gray-50">
              <Typography.Text className="text-xs text-gray-600">
                {t("xOfYSelected", { count: selectedGuardrails.size, total: filteredGuardrails.length })}
              </Typography.Text>
            </div>
          </div>
