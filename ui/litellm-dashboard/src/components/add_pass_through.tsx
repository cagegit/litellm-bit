...
const AddPassThroughEndpoint: React.FC<AddFallbacksProps> = ({
  ...
  const addPassThrough = async (formValues: Record<string, any>) => {
    ...
    try {
      ...
      NotificationsManager.success(t("passThroughCreated"));
      form.resetFields();
      setPathValue("");
      setTargetValue("");
      setIncludeSubpath(true);
      ...
      NotificationsManager.fromBackend(t("errorCreatingPassThrough") + error);
    } finally {
      setIsLoading(false);
    }
  };
  ...
  return (
    <div>
      <Button className="mx-auto mb-4 mt-4" onClick={() => setIsModalVisible(true)}>
        {t("addPassThroughEndpoint")}
      </Button>
      <Modal
        title={
          <div className="flex items-center space-x-3 pb-4 border-b border-gray-100">
            <ApiOutlined className="text-xl text-blue-500" />
            <h2 className="text-xl font-semibold text-gray-900">{t("addPassThroughEndpoint")}</h2>
          </div>
        }
        open={isModalVisible}
        width={1000}
      ...
      >
        <div className="mt-6">
          ...
          >
            ...
            <Card className="p-5">
              <Title className="text-lg font-semibold text-gray-900 mb-2">{t("routeConfiguration")}</Title>
              <Subtitle className="text-gray-600 mb-5">{t("routeConfigDescription")}</Subtitle>

              <div className="space-y-5">
                <Form.Item
                  label={<span className="text-sm font-medium text-gray-700">{t("pathPrefix")}</span>}
                  name="path"
                  rules={[{ required: true, message: t("pathRequired"), pattern: /^\// }]}
                  extra={<div className="text-xs text-gray-500 mt-1">{t("pathPrefixExample")}</div>}
                  className="mb-4"
                >
                ...
                <Form.Item
                  label={<span className="text-sm font-medium text-gray-700">{t("targetUrl")}</span>}
                  name="target"
                  rules={[
                    { required: true, message: t("targetUrlRequired") },
                    { type: "url", message: t("validUrlRequired") },
                  ]}
                  extra={<div className="text-xs text-gray-500 mt-1">{t("targetUrlExample")}</div>}
                  className="mb-4"
                >
                  <TextInput
                ...
                <Form.Item
                  label={
                    <span className="text-sm font-medium text-gray-700 flex items-center">
                      {t("httpMethodsOptional")}
                      <Tooltip title={t("httpMethodsTooltip")}>
                  ...
                  extra={
                    <div className="text-xs text-gray-500 mt-1">
                      {selectedMethods.length === 0
                        ? t("allMethodsDefault")
                        : t("onlyMethodsWillBeRouted", { methods: selectedMethods.join(", ") })}
                    </div>
                  }
                  className="mb-4"
                >
                ...
                <div className="flex items-center justify-between py-3">
                  <div>
                    <div className="text-sm font-medium text-gray-700">{t("includeSubpaths")}</div>
                    <div className="text-xs text-gray-500 mt-0.5">{t("includeSubpathsDescription")}</div>
                  </div>
            ...
            <Card className="p-6">
              ...
              <Subtitle className="text-gray-600 mb-6">{t("headersDescription")}</Subtitle>

              <Form.Item
                label={
                  <span className="text-sm font-medium text-gray-700 flex items-center">
                    {t("authenticationHeaders")}
                    <Tooltip title={t("authHeadersTooltip")}>
                      <InfoCircleOutlined className="ml-2 text-blue-400 hover:text-blue-600 cursor-help" />
                    </Tooltip>
                  </span>
                }
                name="headers"
                rules={[{ required: true, message: t("pleaseConfigureHeaders") }]}
                extra={
                  <div className="text-xs text-gray-500 mt-2">
                    <div className="font-medium mb-1">{t("addAuthTokens")}</div>
                    <div>{t("commonHeaderExamples")}</div>
                  </div>
                }
              >
                <KeyValueInput />
            ...
            <Card className="p-6">
              <Title className="text-lg font-semibold text-gray-900 mb-2">{t("defaultQueryParameters")}</Title>
              <Subtitle className="text-gray-600 mb-6">{t("defaultQueryDescription")}</Subtitle>

              <Form.Item
                label={
                  <span className="text-sm font-medium text-gray-700 flex items-center">
                    {t("queryParamsOptional")}
                    <Tooltip title={t("queryParamsTooltip")}>
                      <InfoCircleOutlined className="ml-2 text-blue-400 hover:text-blue-600 cursor-help" />
                    </Tooltip>
                  </span>
                }
                ...
                extra={
                  <div className="text-xs text-gray-500 mt-2">
                    <div className="font-medium mb-1">{t("queryParamsSentInfo")}</div>
                    <div>{t("queryParamsOverrideInfo")}</div>
                  </div>
                }
              >
                <QueryParamInput />
            ...
            <Card className="p-6">
              ...
              <Subtitle className="text-gray-600 mb-6">{t("billingDescription")}</Subtitle>

              <Form.Item
                label={
                  <span className="text-sm font-medium text-gray-700 flex items-center">
                    {t("costPerRequest")}
                    <Tooltip title={t("costPerRequestTooltip")}>
                      <InfoCircleOutlined className="ml-2 text-gray-400 hover:text-gray-600" />
                    </Tooltip>
                  </span>
                }
                name="cost_per_request"
                extra={<div className="text-xs text-gray-500 mt-2">{t("costPerRequestHelp")}</div>}
              >
            ...
            <div className="flex items-center justify-end space-x-3 pt-6 border-t border-gray-100">
              ...
              >
                {isLoading ? t("creating") : t("addPassThroughEndpoint")}
              </Button>
            </div>
          </Form>
        </div>
