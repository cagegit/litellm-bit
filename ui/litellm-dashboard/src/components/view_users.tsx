...
const ViewUserDashboard: React.FC<ViewUserDashboardProps> = ({
  ...
  const handleResetPassword = async (userId: string) => {
    ...
    try {
      ...
      NotificationsManager.fromBackend(t("failedToGeneratePasswordResetLink"));
    }
  };
  ...
  const confirmDelete = async () => {
    if (userToDelete && accessToken) {
      try {
        ...
        NotificationsManager.fromBackend(t("failedToDeleteUser"));
      } finally {
  ...
  const handleEditSubmit = async (editedUser: any) => {
    ...
    try {
      ...
      NotificationsManager.success(t("userUpdatedSuccessfully", { userId: editedUser.user_id }));
    } catch (error) {
  ...
  const handleBulkEdit = () => {
    if (selectedUsers.length === 0) {
      NotificationsManager.fromBackend(t("pleaseSelectUsersToEdit"));
      return;
    }
    ...
    <div className="w-full p-8 overflow-hidden">
            ...
            <>
              ...
              {isProxyAdmin && selectionMode && (
                <Button
                  ...
                  className="flex items-center"
                >
                  {t("bulkEditSelected", { count: selectedUsers.length })}
                </Button>
              )}
            </>
          ) : null}
      ...
      <DeleteResourceModal
        ...
        resourceInformation={[
          ...
          { label: t("userId"), value: userToDelete?.user_id, code: true },
          {
            label: t("globalProxyRole"),
            value:
              (userToDelete && possibleUIRoles?.[userToDelete.user_role]?.ui_label) || userToDelete?.user_role || "-",
          },
          { label: t("totalSpendUsd"), value: userToDelete?.spend?.toFixed(2) },
        ]}
