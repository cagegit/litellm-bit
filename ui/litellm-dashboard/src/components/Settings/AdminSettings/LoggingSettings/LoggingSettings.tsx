...
const LoggingSettings: React.FC = () => {
  ...
  const handleFormSubmit = (formValues: StoreRequestInSpendLogsParams) => {
    ...
    const submitUpdate = () =>
      mutate(updateParams, {
        onSuccess: () => NotificationsManager.success(t("spendLogsUpdated")),
        onError: (error) => NotificationsManager.fromBackend(t("failedToSaveSpendLogs") + parseErrorMessage(error)),
      });
    ...
    <Card title={t("loggingSettings")}>
      <Space direction="vertical" size="large" style={{ width: "100%" }}>
        ...
        >
          ...
          <Form.Item
            label={t("maximumRetentionPeriod")}
            ...
            tooltip={
              ...
              t("retentionPeriodTooltip")
            }
          >
            {isLoadingConfig ? (
              <Skeleton.Input active block />
            ) : (
              <Input placeholder={t("retentionPeriodPlaceholder")} prefix={<ClockCircleOutlined />} />
            )}
          </Form.Item>

          <Form.Item>
            <Button type="primary" htmlType="submit" loading={isPending || isDeletingField} disabled={isLoadingConfig}>
              {isPending || isDeletingField ? t("saving") : t("saveSettings")}
            </Button>
          </Form.Item>
        </Form>
      </Space>
