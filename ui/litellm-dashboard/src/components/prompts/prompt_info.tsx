...
const PromptInfoView: React.FC<PromptInfoProps> = ({ promptId, onClose, accessToken, isAdmin, onDelete, onEdit }) => {
  ...
  const fetchPromptInfo = async (environment?: string) => {
    try {
      ...
      NotificationsManager.fromBackend(t("failedLoadInfo"));
  ...
  const handleDeleteConfirm = async () => {
    ...
    try {
      ...
      NotificationsManager.success(t("deleteSuccess", { promptId: basePromptId }));
      onDelete?.();
      onClose();
    } catch (error) {
      console.error("Error deleting prompt:", error);
      NotificationsManager.fromBackend(t("failedDelete"));
    } finally {
      setIsDeleting(false);
  ...
  const handleVersionClick = async (version: PromptSpec) => {
    ...
    try {
      ...
      NotificationsManager.fromBackend(t("failedLoadVersion", { version: versionNum }));
    }
  };
    ...
    <div className="p-4">
      ...
      {isViewingOldVersion && (
        <div className="mb-4 p-3 bg-amber-50 border border-amber-200 rounded-lg flex items-center justify-between">
          <Text className="text-amber-800">{t("viewingOldVersion", { selectedVersion, latestVersion })}</Text>
          <TremorButton
            variant="light"
            size="xs"
            onClick={() => {
      ...
      <TabGroup>
        <TabList className="mb-4">
          ...
          {promptTemplate ? <Tab key="prompt-template">{t("promptTemplate")}</Tab> : <></>}
          <Tab key="raw-json">{t("rawJson")}</Tab>
        </TabList>
          ...
          <TabPanel>
            <Grid numItems={1} numItemsSm={2} numItemsLg={4} className="gap-4">
              ...
              <Card>
                ...
                <div className="mt-2">
                  ...
                  <Text className="text-xs">{t("updatedLabel", { date: formatDate(promptData.updated_at) })}</Text>
                </div>
              </Card>
            </Grid>
            ...
            <Card className="mt-6">
              ...
              {loadingVersions ? (
                ...
                <Text className="text-gray-400">{t("noVersionsFound", { environment: selectedEnv ?? "" })}</Text>
              )}
            </Card>
          </TabPanel>
            ...
            <TabPanel>
              <Card>
                <div className="flex justify-between items-center mb-4">
                  ...
                  >
                    {copiedStates["prompt-content"] ? t("copied") : t("copyContent")}
                  </Button>
                </div>

                <div className="space-y-4">
                  <div>
                    <Text className="font-medium">{t("templateId")}</Text>
                    <div className="font-mono text-sm bg-gray-50 p-2 rounded">{promptTemplate.litellm_prompt_id}</div>
                  </div>

                  <div>
                    <Text className="font-medium">{t("content")}</Text>
                  ...
                  {promptTemplate.metadata && Object.keys(promptTemplate.metadata).length > 0 && (
                    <div>
                      <Text className="font-medium">{t("templateMetadata")}</Text>
          ...
          <TabPanel>
            <Card>
              <div className="flex justify-between items-center mb-4">
                ...
                >
                  {copiedStates["raw-json"] ? t("copied") : t("copyJson")}
                </Button>
              </div>
      ...
      >
        <p>
          {t("confirmDeletePrompt")} <strong>{basePromptId}</strong>?
        </p>
        <p>{t("cannotUndo")}</p>
      </Modal>
    </div>
