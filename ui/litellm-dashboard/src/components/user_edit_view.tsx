...
export function UserEditView({
  ...
  return (
    <Form form={form} onFinish={handleSubmit} layout="vertical">
      {!isBulkEdit && (
        <Form.Item label={t("userId")} name="user_id">
          <TextInput disabled />
        </Form.Item>
      )}
      ...
      <Form.Item label={t("userAlias")} name="user_alias">
        <TextInput />
      </Form.Item>
      ...
      <Form.Item
        label={
          <span>
            {t("personalModels")}{" "}
            <Tooltip title={t("personalModelsTooltip")}>
        ...
        rules={[
          {
            validator: (_, value) => {
              if (!unlimitedBudget && (value === "" || value === null || value === undefined)) {
                return Promise.reject(new Error(t("pleaseEnterBudget")));
              }
      ...
      <Form.Item label={t("metadata")} name="metadata">
        <Textarea rows={4} placeholder={t("enterMetadataAsJSON")} />
      </Form.Item>
