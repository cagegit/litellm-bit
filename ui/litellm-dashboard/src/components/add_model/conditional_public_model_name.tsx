...
const ConditionalPublicModelName: React.FC = () => {
  ...
  const publicNameTooltipContent = (
    <>
      <div className="mb-2 font-normal">{t("publicModelNameTooltip")}</div>
      <div className="mb-2 font-normal">
        <strong>{t("example")}:</strong> {t("ifYouNameYourPublicModel")}{" "}
        <code className="bg-gray-700 px-1 py-0.5 rounded text-xs">example-name</code>
        {t("andChoose")} <code className="bg-gray-700 px-1 py-0.5 rounded text-xs">openai/qwen-plus-latest</code>
        {t("asTheLiteLLMModel")}
      </div>
      <div className="mb-2 font-normal">
  ...
  const liteLLMModelTooltipContent = <div>{t("liteLLMModelNameTooltip")}</div>;
  ...
  return (
    <>
      <Form.Item
        label={t("modelMappings")}
        name="model_mappings"
        tooltip={t("modelMappingsTooltip")}
        labelCol={{ span: 10 }}
        wrapperCol={{ span: 16 }}
        labelAlign="left"
        rules={[
          {
            ...
            validator: async (_, value) => {
              if (!value || value.length === 0) {
                throw new Error(t("atLeastOneMappingRequired"));
              }
              ...
              if (invalidMappings.length > 0) {
                throw new Error(t("allMappingsMustHaveValidNames"));
              }
            },
          },
        ]}
