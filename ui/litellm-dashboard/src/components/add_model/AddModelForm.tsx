...
const AddModelForm: React.FC<AddModelFormProps> = ({
  ...
  return (
    <>
      <Title level={2}>{t("addModel")}</Title>
            ...
            {isTeamAdmin && !isAdmin && (
              <>
                <Form.Item
                  label={t("selectTeam")}
                  name="team_id"
                  rules={[{ required: true, message: t("pleaseSelectTeamToContinue") }]}
                  tooltip={t("selectTeamTooltip")}
                >
                  <TeamDropdown
                    onChange={(value) => {
                ...
                {!teamAdminSelectedTeam && (
                  <Alert
                    message={t("teamSelectionRequired")}
                    description={t("teamSelectionRequiredDescription")}
                    type="info"
                    showIcon
                    className="mb-4"
                  />
              ...
              <>
                ...
                >
                  <AntdSelect
                    ...
                    placeholder={isProviderMetadataLoading ? t("loadingProviders") : t("selectAProvider")}
                ...
                <Row>
                  ...
                  <Col span={10}>
                    <Text className="mb-5 mt-1">
                      <strong>{t("optional")}</strong> - {t("liteLLMEndpointDescription")}{" "}
                ...
                <div className="mb-4">
                  <Typography.Text className="text-sm text-gray-500 mb-2">
                    {t("selectOrEnterCredentials")}
                  </Typography.Text>
                </div>

                <Form.Item label={t("existingCredentials")} name="litellm_credential_name" initialValue={null}>
                  <AntdSelect
                    showSearch
                ...
                <div className="flex items-center my-4">
                  <div className="flex-grow border-t border-gray-200"></div>
                  <span className="px-4 text-gray-500 text-sm">{t("additionalModelInfoSettings")}</span>
                  <div className="flex-grow border-t border-gray-200"></div>
                </div>
                {/* Team-only Model Switch - Only show for proxy admins, not team admins */}
                {(isAdmin || !isTeamAdmin) && (
                  <Form.Item label={t("teamBYOKModel")} tooltip={t("teamBYOKModelTooltip")} className="mb-4">
                    <Tooltip title={!premiumUser ? t("enterpriseOnlyFeature") : ""} placement="top">
                      <Switch
                        checked={isTeamOnly}
                        onChange={(checked) => {
                ...
                {isTeamOnly && (isAdmin || !isTeamAdmin) && (
                  <Form.Item
                    ...
                    name="team_id"
                    className="mb-4"
                    tooltip={t("teamModelTooltip")}
                    rules={[
                      {
                        required: isTeamOnly && !isAdmin,
                        message: t("pleaseSelectATeam"),
                      },
                    ]}
                  >
                ...
                {isAdmin && (
                  <>
                    <Form.Item
                      ...
                      className="mb-4"
                      tooltip={t("modelAccessGroupTooltip")}
                    >
                      <AntdSelect
                        mode="tags"
                        showSearch
