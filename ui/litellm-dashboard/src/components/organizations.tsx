...
const OrganizationsTable: React.FC<OrganizationsTableProps> = ({
  ...
  const confirmDelete = async () => {
    ...
    try {
      ...
      NotificationsManager.success(t("organizationDeleted"));
  ...
  const handleCreate = async (values: any) => {
    try {
      ...
      NotificationsManager.success(t("organizationCreated"));
  ...
  return (
    <div className="w-full mx-4 h-[75vh]">
          ...
          {selectedOrgId ? (
            ...
            <TabGroup className="gap-2 h-[75vh] w-full">
              <TabList className="flex justify-between mt-2 w-full items-center">
                <div className="flex">
                  <Tab>{t("yourOrganizations")}</Tab>
                </div>
                <div className="flex items-center space-x-2">
                  {lastRefreshed && <Text>{t("lastRefreshed", { lastRefreshed })}</Text>}
                  <Icon
                    icon={RefreshIcon}
                    variant="shadow"
                    size="xs"
              ...
              <TabPanels>
                <TabPanel>
                  <Text>{t("clickToViewDetails")}</Text>
                    ...
                    <Col numColSpan={1}>
                      <Card className="w-full mx-auto flex-auto overflow-hidden overflow-y-auto max-h-[50vh]">
                        ...
                        <Table>
                          <TableHead>
                            <TableRow>
                              <TableHeaderCell>{t("organizationId")}</TableHeaderCell>
                              <TableHeaderCell>{t("organizationName")}</TableHeaderCell>
                              <TableHeaderCell>{t("createdAt")}</TableHeaderCell>
                              <TableHeaderCell>{t("spendUsd")}</TableHeaderCell>
                              <TableHeaderCell>{t("budgetUsd")}</TableHeaderCell>
                              <TableHeaderCell>{t("models")}</TableHeaderCell>
                              <TableHeaderCell>{t("tpmRpmLimits")}</TableHeaderCell>
                              ...
                              ? organizations
                                  ...
                                  .map((org: Organization) => (
                                    <TableRow key={org.organization_id}>
                                      ...
                                      <TableCell>
                                        {org.created_at ? new Date(org.created_at).toLocaleDateString() : t("na")}
                                      </TableCell>
                                                      ...
                                                      {org.models.length > 3 &&
                                                        !expandedAccordions[org.organization_id || ""] && (
                                                          <Badge size={"xs"} color="gray" className="cursor-pointer">
                                                            <Text>
                                                              +{org.models.length - 3}{" "}
                                                              {t("moreModels", { count: org.models.length - 3 })}
                                                            </Text>
                                                          </Badge>
                                                        )}
                                                      {expandedAccordions[org.organization_id || ""] && (
                                      ...
                                      <TableCell>
                                        <Text>{t("members", { count: org.members?.length || 0 })}</Text>
                                      </TableCell>
                                      <TableCell>
                                        {userRole === "Admin" && (
                                          <>
                                            <TableIconActionButton
                                              variant="Edit"
                                              tooltipText={t("editOrganization")}
                                              onClick={() => {
                                            ...
                                            <TableIconActionButton
                                              variant="Delete"
                                              tooltipText={t("deleteOrganization")}
      ...
      >
        <Form form={form} onFinish={handleCreate} labelCol={{ span: 8 }} wrapperCol={{ span: 16 }} labelAlign="left">
          ...
          <Form.Item label={t("resetBudget")} name="budget_duration">
            <Select2 defaultValue={null} placeholder={t("notApplicable")}>
          ...
          <div style={{ textAlign: "right", marginTop: "10px" }}>
            <Button type="submit">{t("createOrganization")}</Button>
          </div>
        </Form>
      </Modal>
      ...
      <DeleteResourceModal
        ...
        resourceInformationTitle={t("organizationInformation")}
        resourceInformation={[{ label: t("organizationId"), value: orgToDelete, code: true }]}
