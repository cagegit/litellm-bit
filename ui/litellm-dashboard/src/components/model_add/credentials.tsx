...
const CredentialsPanel: React.FC<CredentialsPanelProps> = ({ uploadProps }) => {
  ...
  const handleUpdateCredential = async (values: any) => {
    ...
    NotificationsManager.success(t("credentialUpdated"));
  ...
  const handleAddCredential = async (values: any) => {
    ...
    NotificationsManager.success(t("credentialAdded"));
  ...
  const handleDeleteCredential = async () => {
    ...
    try {
      ...
      NotificationsManager.success(t("credentialDeleted"));
      await refetchCredentials();
    } catch (error) {
      NotificationsManager.error(t("failedToDeleteCredential"));
    } finally {
  ...
  return (
    <div className="w-full mx-auto flex-auto overflow-y-auto p-2">
      {canModifyCredentials && <Button onClick={() => setIsAddModalOpen(true)}>{t("addCredential")}</Button>}
      <div className="flex justify-between items-center mt-4 mb-4">
        <Text>{t("credentialsDescription")}</Text>
      </div>
      ...
      <Card>
        <Table>
          <TableHead>
            <TableRow>
              <TableHeaderCell>{t("credentialName")}</TableHeaderCell>
      ...
      <DeleteResourceModal
        ...
        message={t("deleteCredentialMessage")}
        resourceInformationTitle={t("credentialInformation")}
        resourceInformation={[
