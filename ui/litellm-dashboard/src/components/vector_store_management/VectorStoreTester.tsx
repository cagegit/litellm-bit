...
export const VectorStoreTester: React.FC<VectorStoreTesterProps> = ({ vectorStoreId, accessToken, className = "" }) => {
  ...
  const handleSearch = async () => {
    if (!query.trim()) {
      MessageManager.warning(t("pleaseEnterSearchQuery"));
      return;
    }
  ...
  const clearHistory = () => {
    setSearchHistory([]);
    setExpandedResults({});
    NotificationsManager.success(t("searchHistoryCleared"));
  };
        ...
        <div className="flex-1 overflow-auto p-4 pb-0">
          {searchHistory.length === 0 ? (
            <div className="h-full flex flex-col items-center justify-center text-gray-400">
              ...
              <Text>{t("testVectorStoreHint")}</Text>
            </div>
          ) : (
                  ...
                  <div className="text-right">
                    <div className="inline-block max-w-[80%] rounded-lg shadow-sm p-3 bg-blue-50 border border-blue-200">
                      <div className="flex items-center gap-2 mb-1">
                        <strong className="text-sm">{t("query")}</strong>
                  ...
                  <div className="text-left">
                    <div className="inline-block max-w-[80%] rounded-lg shadow-sm p-3 bg-white border border-gray-200">
                      <div className="flex items-center gap-2 mb-2">
                        ...
                        <strong className="text-sm">{t("vectorStoreResults")}</strong>
                        {entry.response && (
                          <span className="text-xs px-2 py-0.5 rounded bg-gray-100 text-gray-600">
                            {entry.response.data?.length || 0} {t("results")}
                          </span>
                        )}
                      </div>
                                ...
                                >
                                  <div className="flex items-center">
                                    ...
                                    <span className="font-medium text-sm">
                                      {t("resultNumber", { number: resultIndex + 1 })}
                                    </span>
                                  ...
                                  <span className="text-xs bg-blue-100 text-blue-800 px-2 py-1 rounded">
                                    {t("score", { score: result.score.toFixed(4) })}
                                  </span>
                                </div>
                                        ...
                                        <div className="space-y-2 text-xs">
                                          {result.file_id && (
                                            <div className="bg-gray-50 p-2 rounded">
                                              <span className="font-medium">{t("fileId")}:</span> {result.file_id}
                                            </div>
                                          )}
                                          {result.filename && (
                                            <div className="bg-gray-50 p-2 rounded">
