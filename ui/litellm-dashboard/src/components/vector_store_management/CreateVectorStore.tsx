...
const CreateVectorStore: React.FC<CreateVectorStoreProps> = ({ accessToken, onSuccess }) => {
  ...
  const uploadProps: UploadProps = {
    ...
    beforeUpload: (file) => {
      ...
      if (!isValidType) {
        MessageManager.error(t("unsupportedFileType", { fileName: file.name }));
        return Upload.LIST_IGNORE;
      }

      const isLt50M = file.size / 1024 / 1024 < 50;
      if (!isLt50M) {
        MessageManager.error(t("fileTooLarge", { fileName: file.name }));
        return Upload.LIST_IGNORE;
      }
  ...
  const handleCreateVectorStore = async () => {
    if (documents.length === 0) {
      MessageManager.warning(t("pleaseUploadAtLeastOne"));
      return;
    }

    if (!selectedProvider) {
      MessageManager.warning(t("pleaseSelectProvider"));
      return;
    }
    ...
    for (const field of requiredFields) {
      if (!providerParams[field.name]) {
        MessageManager.warning(t("pleaseProvideField", { fieldLabel: field.label }));
        return;
      }
    }
    ...
    try {
      ...
      NotificationsManager.fromBackend(t("createFailed", { error: String(error) }));
    } finally {
      setIsCreating(false);
    }
  };
  ...
  return (
    <div className="space-y-6">
      <div>
        <Title>{t("createVectorStore")}</Title>
        <Text className="text-gray-500">{t("createVectorStoreDescription")}</Text>
      </div>

      {/* Upload Area */}
      <Card>
        <div className="mb-4">
          <Text className="font-medium">{t("step1UploadDocuments")}</Text>
          <Text className="text-sm text-gray-500 block mt-1">{t("uploadInstructions")}</Text>
        </div>
        <Dragger {...uploadProps}>
      ...
      {documents.length > 0 && (
        <Card>
          <div className="mb-4">
            <Text className="font-medium">{t("uploadedDocuments", { count: documents.length })}</Text>
          </div>
      ...
      <Card>
        <div className="space-y-4">
          <div>
            <Text className="font-medium">{t("step2ConfigureVectorStore")}</Text>
            <Text className="text-sm text-gray-500 block mt-1">{t("configureVectorStoreDescription")}</Text>
          </div>
          ...
          <Form form={form} layout="vertical">
            <Form.Item
              label={
                <span>
                  {t("vectorStoreName")}{" "}
                  <Tooltip title={t("vectorStoreNameTooltip")}>
            ...
            >
              <Input
                ...
                placeholder={t("vectorStoreNamePlaceholder")}
                size="large"
            ...
            <Form.Item
              label={
                <span>
                  {t("provider")}{" "}
                  <Tooltip title={t("providerTooltip")}>
                    <InfoCircleOutlined style={{ marginLeft: "4px" }} />
                  </Tooltip>
                </span>
              }
                    ...
                    <Select.Option key={providerEnum} value={vectorStoreProviderMap[providerEnum]}>
                      <div className="flex items-center space-x-2">
                        <img
                          ...
                          alt={t("providerLogoAlt", { providerName: providerEnum })}
                          className="w-5 h-5"
                          onError={(e) => {
          ...
          <div className="flex justify-end">
            ...
            >
              {isCreating ? t("creatingVectorStore") : t("createVectorStore")}
            </Button>
          </div>
        </div>
      </Card>
      ...
      {ingestResults.length > 0 && (
        <Alert
          message={t("vectorStoreCreatedSuccess")}
          description={
            <div>
              <p>
                <strong>{t("vectorStoreId")}:</strong> {ingestResults[0]?.vector_store_id}
              </p>
              <p>
                <strong>{t("documentsIngested")}:</strong> {ingestResults.length}
              </p>
            </div>
          }
          type="success"
