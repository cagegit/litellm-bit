      ...
      <Card>
        <div className="space-y-6">
          <div className="flex justify-between items-start">
            <div>
              <Title>{t("summaryByUserAgent")}</Title>
              <Subtitle>{t("performanceMetricsForDifferentUserAgents")}</Subtitle>
            </div>

            {/* User Agent Filter */}
            <div className="w-96">
              <Text className="text-sm font-medium block mb-2">{t("filterByUserAgents")}</Text>
              <Select
                mode="multiple"
                placeholder={t("allUserAgents")}
                value={selectedTags}
                ...
                return (
                  <Card key={index}>
                    ...
                    <div className="mt-4 space-y-3">
                      <div>
                        <Text className="text-sm text-gray-600">{t("successRequests")}</Text>
                        <Metric className="text-lg">{formatAbbreviatedNumber(tag.successful_requests)}</Metric>
                      </div>
                      <div>
                        <Text className="text-sm text-gray-600">{t("totalTokens")}</Text>
                        <Metric className="text-lg">{formatAbbreviatedNumber(tag.total_tokens)}</Metric>
                      </div>
                      <div>
                        <Text className="text-sm text-gray-600">{t("totalCost")}</Text>
      ...
      <Card>
        <TabGroup>
          <TabList className="mb-6">
            <Tab>{t("dauWauMau")}</Tab>
            <Tab>{t("perUserUsageLast30Days")}</Tab>
          </TabList>
          ...
          <TabPanels>
            ...
            <TabPanel>
              <div className="mb-6">
                <Title>{t("dauWauMauPerAgent")}</Title>
                <Subtitle>{t("activeUsersAcrossDifferentTimePeriods")}</Subtitle>
              </div>
              ...
              <TabGroup>
                ...
                <TabPanels>
                  <TabPanel>
                    <div className="mb-4">
                      <Title className="text-lg">{t("dailyActiveUsersLast7Days")}</Title>
                    </div>
                    {dauLoading ? (
                      <ChartLoader isDateChanging={false} />
                    ) : (
                  ...
                  <TabPanel>
                    <div className="mb-4">
                      <Title className="text-lg">{t("weeklyActiveUsersLast7Weeks")}</Title>
                    </div>
                    {wauLoading ? (
                      <ChartLoader isDateChanging={false} />
                    ) : (
                  ...
                  <TabPanel>
                    <div className="mb-4">
                      <Title className="text-lg">{t("monthlyActiveUsersLast7Months")}</Title>
                    </div>
                    {mauLoading ? (
                      <ChartLoader isDateChanging={false} />
                    ) : (
